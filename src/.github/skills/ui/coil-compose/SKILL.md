...
> [!WARNING]
> `rememberAsyncImagePainter` does not detect the size your image is loaded at on screen. Use `AsyncImage` unless a `Painter` is strictly required.
...
> [!CAUTION]
> Avoid using `SubcomposeAsyncImage` in performance-critical areas like `LazyColumn` or `LazyRow`.
...
SubcomposeAsyncImage(
    ...
    error = { Icon(Icons.Default.Error, contentDescription = null) }
)
```
