...
function Skyline() {
  const left = [
    [1, 126, 24], [17, 86, 18], [32, 104, 20], [48, 68, 22], [63, 114, 16],
  ];
  const right = [
    [2, 96, 18], [17, 66, 22], [35, 116, 18], [52, 80, 24], [70, 128, 20],
  ];
  ...
  return (
    <div className="absolute inset-x-[-10px] bottom-0 h-[180px] overflow-hidden">
      <div
        className="absolute inset-x-8 bottom-7 h-20"
        style={{
          background: 'radial-gradient(ellipse at center, rgba(168,85,247,0.5), rgba(44,12,92,0.2) 48%, transparent 74%)',
          filter: 'blur(10px)',
        }}
      />
      <div
      ...
      <div className="absolute inset-x-0 bottom-0 h-14 bg-gradient-to-t from-[#03040e] to-transparent" />
    </div>
  );
}
  ...
  return (
    <div className="relative mx-auto h-[132px] w-[132px]">
      <div
        ...
        style={{
          background: 'radial-gradient(circle, rgba(168,85,247,0.34), rgba(88,28,135,0.12) 48%, transparent 72%)',
        }}
      />
      <motion.div
...
function TimelineArc() {
  ...
  return (
    <div className="relative h-[66px] w-full">
      <svg viewBox="0 0 390 76" className="absolute inset-x-0 top-0 h-12 w-full overflow-visible" aria-hidden="true">
      ...
      {ticks.map(t => (
        <span
          ...
          style={{ left: t.left, height: Math.max(12, t.h - 10), boxShadow: '0 0 10px rgba(216,180,254,0.8)' }}
        />
      ))}
      <motion.span
        className="absolute top-6 h-7 w-7"
      ...
      <div className="absolute inset-x-0 bottom-0 flex justify-between px-5 font-inter text-[13px] font-black text-purple-200/62">
        <span>1800</span>
        <span>1900</span>
...
function HeroStage() {
  return (
    <motion.section
      className="relative mt-1 overflow-visible text-center"
    ...
    >
      <div
        ...
        style={{ height: 'clamp(252px, 36svh, 292px)' }}
      >
        <div
          className="absolute inset-x-[-24px] top-[-18px] h-[238px] pointer-events-none"
          style={{
            background: 'radial-gradient(ellipse at 50% 45%, rgba(168,85,247,0.48) 0%, rgba(80,20,145,0.2) 38%, rgba(3,5,17,0) 73%)',
          }}
        />
        <div
          className="absolute inset-x-[-18px] top-0 h-[168px] pointer-events-none opacity-70"
          style={{
            background: 'linear-gradient(135deg, transparent 0 18%, rgba(99,102,241,0.18) 18% 19%, transparent 19% 44%, rgba(168,85,247,0.16) 44% 45%, transparent 45%), linear-gradient(225deg, transparent 0 22%, rgba(168,85,247,0.16) 22% 23%, transparent 23% 60%)',
            maskImage: 'linear-gradient(to bottom, transparent 0%, black 24%, black 68%, transparent 100%)',
            WebkitMaskImage: 'linear-gradient(to bottom, transparent 0%, black 24%, black 68%, transparent 100%)',
          }}
        />
        <Skyline />
        <div className="absolute inset-x-0 z-10" style={{ top: 'clamp(30px, 5.7svh, 46px)' }}>
          <NeonClock />
        </div>
        <div className="absolute inset-x-0 z-20 px-2" style={{ top: 'clamp(148px, 22svh, 168px)' }}>
          <p className="font-inter text-[20px] font-black leading-none text-white drop-shadow-[0_2px_12px_rgba(255,255,255,0.24)]">
            KARTI DOĞRU YERE KOY,
          </p>
          <p className="mt-2 font-bangers text-[31px] leading-none text-primary drop-shadow-[0_0_17px_rgba(250,204,21,0.72)]">
            ZAMANI SEN YÖNET
          </p>
        </div>
...
function ModeCard({ type, title, subtitle, icon, onClick }) {
  ...
  return (
    <motion.button
      ...
      style={{
        minHeight: 'clamp(124px, 18svh, 142px)',
        background: solo
          ? 'linear-gradient(145deg, rgba(67,18,116,0.88), rgba(18,8,42,0.95))'
          : 'linear-gradient(145deg, rgba(22,17,12,0.94), rgba(9,8,22,0.96))',
        border: solo ? '1.5px solid rgba(216,180,254,0.92)' : '1.5px solid rgba(250,204,21,0.76)',
        boxShadow: solo
          ? '0 0 22px rgba(168,85,247,0.45), inset 0 0 28px rgba(168,85,247,0.14)'
          : '0 0 18px rgba(250,204,21,0.26), inset 0 0 28px rgba(250,204,21,0.08)',
      }}
    >
      ...
      <span
        ...
        style={{
          color: solo ? '#d8b4fe' : '#facc15',
          borderColor: solo ? 'rgba(216,180,254,0.58)' : 'rgba(250,204,21,0.46)',
          boxShadow: solo ? '0 0 18px rgba(168,85,247,0.36)' : '0 0 16px rgba(250,204,21,0.22)',
        }}
      >
        {icon}
      </span>
      <span className="relative block font-bangers text-[24px] leading-[0.96] text-white">
        {title}
      </span>
      <span className="relative mt-3 block font-inter text-[12px] font-bold leading-snug text-white/[0.7]">
        {subtitle}
      </span>
    </motion.button>
  );
  ...
  return (
    <>
      ...
      <div
        ...
        style={{
          background: 'linear-gradient(180deg, #030511 0%, #080420 42%, #05030d 100%)',
          userSelect: 'none',
        }}
      >
        <div
        ...
        <main
          ...
          style={{ paddingTop: 'calc(0.55rem + env(safe-area-inset-top))', paddingBottom: 'calc(0.65rem + env(safe-area-inset-bottom))' }}
        >
          <div className="grid grid-cols-[56px_1fr_56px] items-center gap-2">
            <motion.button
              ...
              style={{
                ...
                boxShadow: '0 0 20px rgba(126,34,206,0.32), inset 0 0 16px rgba(126,34,206,0.12)',
              }}
            ...
            <div className="flex min-w-0 items-center justify-center">
              <motion.img
                ...
                style={{ width: 'clamp(178px, 58vw, 236px)' }}
                ...
                animate={{
                  ...
                  scale: 1,
                  filter: [
                    'drop-shadow(0 0 10px rgba(250,204,21,0.72))',
                    'drop-shadow(0 0 20px rgba(250,204,21,0.95))',
                    'drop-shadow(0 0 10px rgba(250,204,21,0.72))',
                  ],
                }}
            ...
            <motion.button
              ...
              transition={{ type: 'spring', stiffness: 500, damping: 24 }}
              onClick={() => navigate('/settings')}
              className="ml-auto flex h-[52px] w-[52px] items-center justify-center rounded-[18px]"
              style={{
                background: 'rgba(5,7,22,0.68)',
                border: '2px solid rgba(168,85,247,0.56)',
                boxShadow: '0 0 20px rgba(126,34,206,0.32), inset 0 0 16px rgba(126,34,206,0.12)',
              ...
              aria-label="Ayarlar"
            >
              <Settings className="h-6 w-6 text-white drop-shadow-[0_0_10px_rgba(255,255,255,0.7)]" />
            </motion.button>
          </div>
          ...
          >
            <div
              ...
              style={{
                background: 'linear-gradient(180deg, rgba(255,243,126,0.98), rgba(250,204,21,0.88))',
                boxShadow: '0 0 26px rgba(250,204,21,0.56), 0 16px 30px rgba(0,0,0,0.45), inset 0 0 18px rgba(255,255,255,0.24)',
              }}
            >
              <motion.button
                ...
                animate={{
                  boxShadow: [
                    '0 0 18px rgba(250,204,21,0.46), inset 0 4px 0 rgba(255,255,255,0.28), inset 0 -7px 0 rgba(183,118,0,0.22)',
                    '0 0 30px rgba(250,204,21,0.72), inset 0 4px 0 rgba(255,255,255,0.32), inset 0 -7px 0 rgba(183,118,0,0.24)',
                    '0 0 18px rgba(250,204,21,0.46), inset 0 4px 0 rgba(255,255,255,0.28), inset 0 -7px 0 rgba(183,118,0,0.22)',
                  ],
                }}
                transition={{ boxShadow: { duration: 1.8, repeat: Infinity, ease: 'easeInOut' }, scale: { type: 'spring', stiffness: 500, damping: 24 } }}
                className="flex w-full items-center justify-center gap-5 rounded-[20px] font-bangers text-[36px] leading-none"
                style={{
                  height: 'clamp(62px, 8.7svh, 70px)',
          ...
          <section className="mt-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <ModeCard
                ...
                icon={<UserRound strokeWidth={1.7} className="h-7 w-7" />}
                onClick={handleSolo}
              />
              <ModeCard
                type="online"
