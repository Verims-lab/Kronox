...
Hard deck rules:
- 17 questions for before_after onboarding levels
- 18 questions for timeline_basic onboarding levels
- 18 questions for normal levels
- 21 questions for special levels
- before_after onboarding: 1 fixed reference card + 10 attempt cards + safe buffer
- timeline_basic onboarding: 2 fixed reference cards + 10 attempt cards + safe buffer
- onboarding levels use 10 evaluated moves while progress remains 6 correct placements
...
- Zamanı Dondur does not require an extra card
- İpucu opens the active-card Hint popup, pauses the visible Solo timer, reveals
  ...
  Hint spends, and is overlap-aware with Zamanı Dondur so the same frozen seconds
