...
export default function LobbyChat({ lobbyId, playerName, compact = false }) {
  ...
  return (
    <div className={`flex flex-col border border-border/30 rounded-xl bg-secondary/10 overflow-hidden ${compact ? 'h-48' : 'h-full'}`} style={{ overscrollBehavior: 'none' }}>
      ...
      {/* Messages */}
      <div ref={chatScrollRef} className="flex-1 overflow-y-auto px-3 py-2 space-y-1.5" style={{ overscrollBehavior: 'none', transform: pullY > 0 ? `translateY(${pullY}px)` : undefined, transition: pullY === 0 ? 'transform 0.2s' : undefined }}>
        {refreshing && (
