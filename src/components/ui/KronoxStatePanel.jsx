...
export default function KronoxStatePanel({
  ...
  return (
    <div
      ...
      data-kronox-state-panel="mobile-safe"
      className={`w-full min-w-0 max-w-full overflow-hidden rounded-2xl text-center ${compact ? 'px-3 py-3' : 'px-4 py-5'}`}
    ...
    >
      ...
      {title && <p className="mt-2 break-words font-inter text-sm font-black text-white">{title}</p>}
      {message && <p className="mt-1 break-words font-inter text-xs leading-relaxed text-blue-100/70">{message}</p>}
      {onAction && kind !== 'loading' && (
        <button
          type="button"
          onClick={onAction}
          className="mt-3 inline-flex min-h-11 max-w-full items-center justify-center gap-2 rounded-xl px-4 font-inter text-xs font-black text-amber-950"
