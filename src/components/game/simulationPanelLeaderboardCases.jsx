...
export const EXTRA_TESTS = [
  ...
  makeCase('leaderboard_health', 'leaderboard_panel_list_visual_contract',
    ...
    () => {
      ...
      const requiredCss = missingTokens(indexCssSource, [
        ...
        'color: #A7C4E5',
        '.leaderboard-avatar',
        'width: clamp(2.5rem, 11vw, 3.2rem)',
        'aspect-ratio: 1 / 1',
        'border-radius: 50%',
        'object-fit: cover',
        '.leaderboard-avatar--default',
        ...
        'position: fixed',
        'left: 50%',
        'clamp(4.4rem, 10dvh, 5.4rem)',
        'width: min(90%, 32rem)',
        'z-index: 30',
        '.my-rank-card',
        'rgba(49, 43, 25, 0.94) 0%',
        'rgba(17, 31, 57, 0.98) 100%',
        'rgba(255, 201, 40, 0.62)',
        '.my-rank-label',
        'clamp(0.85rem, 3.8vw, 1.05rem)',
        '.my-rank-you-badge',
        'border-radius: 999rem',
        'clamp(0.55rem, 2.5vw, 0.7rem)',
      ]);
      ...
      if (requiredComponent.length || requiredCss.length || forbidden.length) {
        return fail('Liderlik ranking panel/list visual contract drifted.', {
          ...
          classification: 'VISUAL_REGRESSION_RISK',
          actionType: ACTION_TYPES.CODE_FIX,
          expected: 'KronoxRankingSection uses .leaderboard-panel/.leaderboard-section-title/.leaderboard-list/.leaderboard-row/.leaderboard-rank plus scoped avatar/name/score/sticky current-user styles; removed helper/friends copy is absent; row rank/avatar/username/score data remains rendered.',
          actual: { requiredComponent, requiredCss, forbidden },
        });
      }
      return pass('Liderlik ranking panel uses the approved scrollable list, row, avatar/name/score, and sticky current-user style while preserving public row rank, avatar, username, badges, and score.', {
        verification: 'STATIC_CONTRACT',
        classification: 'STATIC_CHECK_LIMITATION',
        actionType: ACTION_TYPES.CODE_FIX,
      });
  ...
  */
  makeCase('leaderboard_health', 'leaderboard_empty_state_safe',
    'Leaderboard has loading, no-user, fallback, and retry states without the removed Friends area',
    () => {
      const combined = `${leaderboardPageSource}\n${kronoxRankingSectionSource}`;
      const required = missingTokens(combined, [
        'Sıralama yükleniyor',
      ...
      if (required.length || forbidden.length) {
        return fail('Leaderboard empty/fallback state coverage drifted.', {
          ...
          classification: 'REAL_PRODUCT_RISK',
          actionType: ACTION_TYPES.CODE_FIX,
          expected: 'non-crashing product copy for loading/empty/finalizing/retry, with the old Leaderboard Friends empty area removed',
          actual: { required, forbidden },
        });
      }
      return pass('Leaderboard exposes safe product-copy states for loading, empty, finalizing, and retry without the removed Friends empty area.', {
        verification: 'STATIC_CONTRACT',
        classification: 'STATIC_CHECK_LIMITATION',
        actionType: ACTION_TYPES.CODE_FIX,
      });
