...
const pick = (used, pool, years = new Set(), recent = new Set()) => { const available = pool.filter(q => !used.has(q.id)); if (!available.length) return null; let preferred = available.filter(q => !years.has(q.year) && !recent.has(q.id)); if (preferred.length < 5) preferred = available.filter(q => !years.has(q.year)); return (preferred.length ? preferred : available)[0] || null; };
...
const TESTS = [
  ...
  sourceHas('offline', 'personal_record_better_only', 'personal record update path exists', 'Game.jsx/TopScores', `${SRC.Game}\n${SRC.Settings}`, ['GameRecord', 'TopScores']),
  ...
  sourceHas('exceptional', 'failed_lobby_update_safe', 'failed Lobby.update does not corrupt local state', 'Game.jsx', SRC.Game, ['try {', 'Lobby.update', 'catch']),
