...
export const OVERRIDDEN_CASE_KEYS = new Set([
  ...
  // Backend deployability incident — the deployed root flat function
  // functions/sendQuestionAnalyticsReportEmail.js previously imported
  ...
  // the flat function deploys cleanly. The old Health case still required the
  // broken './_shared/adminAuth.js' token, so it is stale and overridden below.
...
export const EXTRA_TESTS = [
  ...
  /* ==================================================================
   *  Backend deployability incident — deployed root report entrypoint
   *  must deploy cleanly under the Base44 flat-function runtime.
   *
   *  Root cause that was fixed: the flat function imported
   *  './_shared/adminAuth.js' which resolved to file:///src/_shared/adminAuth.js
   ...
   *
   *  New contract: the deployed flat function INLINES a DB-backed AdminUser
   ...
   *  import in the flat deploy path so the broken pattern cannot return.
   * ================================================================= */
  makeCase(
    'question_analytics_health', 'Question Analytics Health Suite',
    'manual_admin_email_report_deployed_root_entrypoint',
    'Deployed root report entrypoint deploys cleanly (inline AdminUser guard, no local import) and keeps the static-pool-v2 contract',
    () => {
      ...
      // never come back in the executed flat-root path.
      const forbidden = [
      ...
      if (missing.length || found.length || !orderOk) {
        return fail('Deployed flat report entrypoint can regress to a non-deployable local import or lose the static-pool-v2 contract.', {
          verification: 'STATIC_CONTRACT',
          classification: 'REAL_PRODUCT_RISK',
      ...
      return pass('Deployed flat report entrypoint inlines the AdminUser guard, has no local import, and keeps the static Question-table chart before long event sections.', {
        verification: 'STATIC_CONTRACT',
        classification: 'STATIC_CHECK_LIMITATION',
        file: 'functions/sendQuestionAnalyticsReportEmail.js',
        actionType: ACTION_TYPES.CODE_FIX,
