...
//   A serious incident: the deployed flat-root function
//   functions/sendQuestionAnalyticsReportEmail.js imported
//   './_shared/adminAuth.js', which resolved to file:///src/_shared/adminAuth.js
//   (module not found) and FAILED to deploy under the Base44 flat-function
//   runtime. Because the deploy failed, Base44 kept serving a STALE build, so
//   the real email report was missing new sections even though local proof
//   HTML and `npm run build` (Vite frontend only) looked correct.
//
//   This suite statically guards the DEPLOYED flat-root functions/ path
//   against that class of regression:
//     • No flat-root function may import the local shared admin guard via the
//       broken './_shared/adminAuth.js' / './_shared/adminAuth.ts' form.
//     • The analytics report flat function must inline an AdminUser-backed
//       guard (proven deployable) and keep the static-pool-v2 markers.
...
import deployedReportSource from '../../../functions/sendQuestionAnalyticsReportEmail.js?raw';
...
function makeCase(id, name, run, options = {}) {
  return {
    ...
    nextStep: options.nextStep
      || 'Keep deployed flat-root functions free of broken local imports; prove real backend deploy by triggering the function and reading its live markers.',
    ...options,
    run,
  };
}
...
export const EXTRA_TESTS = [
  makeCase('deployed_report_has_no_broken_local_import',
    'Deployed analytics report flat function has no broken local _shared import',
    () => {
      ...
      if (forbidden.length) {
        return fail('Deployed flat report function still imports the local shared admin guard, which breaks Base44 flat-function deployment.', {
          verification: 'STATIC_CONTRACT',
          classification: 'REAL_PRODUCT_RISK',
          file: 'functions/sendQuestionAnalyticsReportEmail.js',
          expected: 'inline AdminUser-backed guard; no local _shared import in the flat deploy path',
          actual: { foundForbidden: forbidden },
          actionType: ACTION_TYPES.CODE_FIX,
        });
      }
      return pass('Deployed flat report function has no broken local _shared import — it deploys under the flat-function runtime.', {
        verification: 'STATIC_CONTRACT',
  ...
  makeCase('deployed_report_inlines_admin_user_guard',
    'Deployed analytics report flat function inlines a DB-backed AdminUser guard',
    () => {
      const required = [
      ...
      if (missing.length || !guardBeforeSend) {
        return fail('Deployed report flat function admin guard is missing, not DB-backed, or runs after report work.', {
          verification: 'STATIC_CONTRACT',
          classification: 'REAL_PRODUCT_RISK',
          file: 'functions/sendQuestionAnalyticsReportEmail.js',
      ...
      return pass('Deployed report flat function authorizes admins from an inline DB-backed AdminUser guard before doing any report work.', {
        verification: 'STATIC_CONTRACT',
        classification: 'STATIC_CHECK_LIMITATION',
        file: 'functions/sendQuestionAnalyticsReportEmail.js',
      });
  ...
  makeCase('deployed_report_keeps_static_pool_markers',
    'Deployed analytics report flat function keeps static-pool-v2 markers and body diagnostics',
    () => {
      const required = [
      ...
      if (missing.length || !orderOk) {
        return fail('Deployed report flat function lost the static-pool-v2 template markers or the static section ordering.', {
          verification: 'STATIC_CONTRACT',
          classification: 'REAL_PRODUCT_RISK',
          file: 'functions/sendQuestionAnalyticsReportEmail.js',
      ...
      return pass('Deployed report flat function keeps static-pool-v2 markers, body diagnostics, and the static section before long event sections.', {
        verification: 'STATIC_CONTRACT',
        classification: 'STATIC_CHECK_LIMITATION',
        file: 'functions/sendQuestionAnalyticsReportEmail.js',
      });
