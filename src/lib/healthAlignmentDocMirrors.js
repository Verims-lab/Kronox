...
- Data Hygiene duplicate cleanup is a two-step process: first an AdminUser-gated read-only dry-run plan, then a separate explicitly approved task. The plan proposes canonical candidates, P0/P1 risk, REVIEW_REQUIRED/DO_NOT_AUTOMATE eligibility, and bounded irreversible fingerprints only. Cleanup execution is separate and blocked until explicit admin/user approval. Duplicate checks remain FAIL until separately approved execution runs and a fresh report verifies them; no raw rows or private IDs reach UI.
