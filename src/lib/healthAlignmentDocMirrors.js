...
- Online offers Arkadaşını Davet Et (invite mode, 60s Pre-game Hourglass wait), Rastgele Eşleş (random matchmaking queue, 30s Pre-game Hourglass wait), and Duello as the final display name for the stable internal same_question_duel key. Optional manual invite/social-player-list load failure is local to the invite card only and must never block the Online page or disable Rastgele Eşleş.
...
## Duello V1
Two real actors must enter the mode-scoped same_question_duel queue, receive the same sequence/prompt without an answer year or raw Question ids, and submit near-simultaneous correct placements. Exactly one backend claim may win; the other client must reconcile to the newer revision with the safe copy "Bu kart rakip tarafından alındı." Verify first-to-10 terminal state, one-time winner +15 / loser -6 persistence, cancel/timeout cleanup, reconnect recovery, and no private/full-deck fields. The Duello Health Suite is a source-connected contract guard; this deployed two-device race remains MANUAL_EXTERNAL.
