...
- GuestProfile public identity uses username; display_name is only a legacy/internal projection mirror. Email, Google ID, Apple ID, provider UID, raw guest id, internal owner_key, and internal player_key values are not public display names.
...
- Online question selection, getQuestions, and analytics do not read preferences for question selection. Online start uses startLobbyGame to persist a bounded shared online_question_deck on Lobby, selected 100% from active lobby-selected categories with difficulty 1/2 only; missing/invalid selected categories or Category read failures must not fall back to legacy category names, Lobby.category, or old seeded arrays; Game reads that persisted deck instead of the Solo getQuestions buffer.
...
Home / Ana Sayfa does not show Google / Apple / email login buttons or a secure-progress / Hesabını bağla account-link card; guest account linking is available from Profile instead.
