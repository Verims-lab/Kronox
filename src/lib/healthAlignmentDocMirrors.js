...
- sendQuestionAnalyticsReportEmail live deploy is proven by triggering the function and reading reportBuildMarker (current: Codex307), templateVersion static-pool-v2, and bodyContainsStaticPoolSection/Template/QuestionSource = true. A published frontend that does not change reportBuildMarker means the executed backend function did not redeploy.
...
iOS AppIcon PNGs must be fully opaque before App Store upload. No alpha channel, no tRNS transparency chunk, and no transparent corners are allowed. The 1024x1024 ios-marketing / large app icon must be RGB/opaque. App Store Connect error 90717 means a transparent or alpha-channel icon remains in the native asset catalog. Run npm run check:ios-icons before native archive upload; it validates ios/App/App/Assets.xcassets/AppIcon.appiconset/Contents.json, referenced PNG dimensions, and no-alpha PNG metadata. Real App Store Connect re-upload validation remains manual.
