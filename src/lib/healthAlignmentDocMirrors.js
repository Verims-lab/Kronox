...
- The deploy manifest/compile gate covers exactly 50 Base44 function entries, rejects removed legacy/test/diagnostic callables, and exact-pins the frontend package plus Deno imports to @base44/sdk 0.8.34. This Base44 workspace does not expose package-lock.json, so lockfile root/resolution proof remains an external package-layer gate and Health must not claim it. Current P0 paths include createGuestProfile, getQuestions, findLobbyByCode, startLobbyGame, updateLobbyGameState, getOnlinePlayerSelection, createGameInvitesForTargets, sendFriendRequest, updatePlayerPresence, consumeUserHint, spendUserJoker, getDailyQuestStatus, recordDailyQuestProgress, claimDailyQuestReward, and admin duplicate report/cleanup functions.
...
spec exactly at 0.8.34, matching backend Deno imports. package-lock.json is not
exposed in this Base44 workspace, so lockfile resolution remains external proof. Transient Friends, Online lobby, and debug copy/
...
- Security Pass 1 pins package.json @base44/sdk exactly at 0.8.34 and aligns Base44 Deno function imports to npm:@base44/sdk@0.8.34. package-lock.json is unavailable in this Base44 workspace, so lockfile root/resolution remains external package-layer proof; Health must not claim it. Do not reintroduce frontend ^ SDK ranges, unversioned function SDK imports, or npm:@base44/sdk@0.8.25 without a documented Base44 runtime compatibility split.
