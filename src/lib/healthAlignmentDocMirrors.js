...
- Paket B4 adds one guarded Admin-only, read-only Yayın Hazırlığı tracker. It shows the 50-function ceiling, SDK/package-lock boundary, build marker, targeted Health requirements, manual device/RLS/VAPID/index/deploy/store gates, and links to B1/B3 without reading secrets/private rows, persisting status, mutating data, claiming Full Health PASS, or triggering deployment.
...
- Paket B4 Yayın Hazırlığı must fit 320/360/390px inside guarded Admin Ekranı; its device rows are checklist visibility only and do not claim Android/iOS/WebView proof.
...
The B4 Yayın Hazırlığı panel is an Admin-only read-only tracker, not proof itself. It stores no checklist status, renders no secret/private values, triggers no deployment, keeps Full Health unfaked, and cannot mark release complete while manual/external gates remain.
...
- Paket B4 adds no entity, function, index, schema, persistence, scheduler, cleanup, or deployment action. Its Admin-only read-only tracker keeps function count 50 and points to B1/B3 while package-lock, platform indexes, production secrets, RLS, devices, and live deployment remain external/manual.
