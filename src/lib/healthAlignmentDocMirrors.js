...
- Paket B4 release readiness renders safe requirement labels only. Production VAPID provisioning, RLS/multi-account isolation, platform indexes, package-lock, live deployment, devices, and store validation remain MANUAL_REQUIRED/external; no secret value, private row, raw error, deploy action, or persistent checklist state is exposed.
- Paket B5 keeps those boundaries open, aligns editable SDK source to exact 0.8.34, and treats unavailable package-lock resolution as external proof rather than a crash or fabricated pass.
- Paket B6 Health automation remains guarded Admin-only, reads no production secret values, mutates no RLS/index/data, classifies proof/fix ownership, and keeps production security plus scheduled monitoring manual/external.
- sendGameInvitePush fails closed (skips push, keeps the in-app invite) when VAPID_PRIVATE_KEY or VAPID_PUBLIC_KEY is missing, empty, or placeholder-like (changeme/test/demo/placeholder/dummy/example/etc.); no hardcoded, default, or generated-at-runtime private key exists, and VAPID_PRIVATE_KEY is never logged, thrown in an error, or returned in any response. Production VAPID env provisioning remains a manual release-proof item.
...
Friend/game invite notifications use public username labels only, preserve valid pending rows through transient empty refreshes, and close only on explicit user action, terminal status, expiry, or confirmed source invalidation.
  Terminal accepted-invite history is bootstrap-suppressed, freshness-filtered, test-marker suppressed, deduped/collapsed into one bounded summary, and globally capped at four visible toasts. Existing artifacts are reported through AdminUser-gated read-only dry-run fingerprints/counts, never auto-deleted or matched destructively by KronoxUser name.
