...
active /lobby evidence fails. A/B Runtime E2E contexts compare only redacted
...
- Duello is the final user-facing display name for the stable internal same_question_duel key; it remains a two-player random-opponent Online mode and is not a BottomNav item. Its public projection uses synthetic card/timeline ids, withholds the active answer year and raw Question/used-question ids, and exposes only the actor's resolved timeline years plus opponent claimed_count.
...
- Duello is the final display name for the stable internal same_question_duel key. Duello V1 is random-opponent only, exactly two-player, mode-scoped from normal random_online, server-authored, and first-correct backend-claimed. randomMatchmaking uses newest-first active lock/queue reads, fails closed on queue reads, rejects self-match, permits guest/guest, guest/linked, and linked/linked actors, and reconciles both actors into one private backend session. Timeout runs a final backend read before mode-locked cleanup; matched clients show Rakip bulundu on /online and enter /duel directly after the sanitized GAME snapshot is ready. Both players receive identical two opening anchors and one monotonic active sequence. Wrong answers lock that player for the current sequence; two wrong answers release the next card. Stale claims return the current backend snapshot and the client accepts only non-regressing state_revision values. First to 10 claims wins. Joker/Hint are disabled, result scoring remains backend winner +15 / loser -6 with no speed bonus, and no category selector or Solo buffer is used. Public snapshots expose a synthetic-id active prompt without its answer year, empty used-question ids, self-only resolved timeline years under synthetic ids, and opponent claimed_count without opponent card rows; real two-device race/reconnect proof remains MANUAL_EXTERNAL.
...
## Duello V1
Two real actors on two physical phones must enter the mode-scoped same_question_duel queue within 30 seconds, show Rakip bulundu on the same /online search surface, enter /duel directly from the same private backend session without /lobby, receive the same sequence/prompt without an answer year or raw Question ids, and submit near-simultaneous correct placements. Exactly one backend claim may win; the other client must reconcile to the newer revision with the safe copy "Bu kart rakip tarafından alındı." Verify guest/guest and guest/linked where possible, timeout/retry/cancel cleanup, a second attempt without a ghost row, normal random_online pairing after the shared-infrastructure repair, first-to-10 terminal state, one-time winner +15 / loser -6 persistence, reconnect recovery, and no private/full-deck fields. The Duello and Online Matchmaking Runtime Health suites are contract guards; this deployed two-device race remains MANUAL_EXTERNAL.
