...
- Duello is the final user-facing display name for the stable internal same_question_duel key; it remains a two-player random-opponent Online mode and is not a BottomNav item.
...
- Duello is the final display name for the stable internal same_question_duel key. Duello V1 is random-opponent only, exactly two-player, mode-scoped from normal random Online, server-authored, and first-correct backend-claimed. Both players receive identical two opening anchors and one functional active shared card at a time. Wrong answers lock that player for the current sequence; two wrong answers release the next card. First to 10 claims wins. Joker/Hint are disabled, result scoring remains backend winner +15 / loser -6, no category selector or Solo buffer is used, public snapshots expose only the active shared card and safe participant/progress fields, and two-device race/reconnect proof remains manual.
