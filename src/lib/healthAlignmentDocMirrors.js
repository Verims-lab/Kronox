...
- Online offers Arkadaşını Davet Et, Online Kapış (canonical random_online queue), and Duello (canonical same_question_duel queue). Random modes search for at most 30 seconds on /online, show Rakip bulundu / Oyun başlıyor on the same surface, then enter /game or /duel directly after a participant-scoped authoritative GAME snapshot is ready. The private Lobby row remains a backend session store only; no waiting-room, ready, manual-start, or active /lobby step exists. Normal random and Duello use separate mode-scoped queue lanes and pairing locks; token-proven guests and linked actors are compatible, self-match is forbidden, and cancel/timeout/consumption are backend-settled terminal states. Optional friend-list failure is local and never disables Online Kapış.
...
- Online requires a completed linked or token-proven guest actor and KRONOX_E2E_ALLOW_MATCHMAKING=true; production search/cancel may run as a runtime probe but requires observed matchmaking backend evidence. Matched proof must remain on /online, reject /lobby, and enter /game directly. Duello A/B actor storage may be declared through KRONOX_E2E_STORAGE_STATE_A/B, but Duello remains MANUAL_EXTERNAL until deterministic pairing and correct-claim fixtures prove direct /duel, one backend winner, and reconciled snapshots. Full Run does not execute E2E and E2E counters never alter Health blocker/fail/warning counts.
`;
