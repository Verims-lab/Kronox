...
- Paket B5 closes source-fixable Paket B gaps: editable SDK source is exact 0.8.34, unavailable lock resolution remains external, stale tutorial preload/Online docs are aligned, public player fallback is username-only, and Paket B Closure Health targets active B1-B4 sources. Manual release gates remain open.
...
root and resolved SDK are also exact 0.8.34; embedded Base44 Health keeps lockfile
resolution as external package-layer proof. Transient Friends, Online lobby, and debug copy/
...
- Base44 Deno function imports and the existing deploy gate remain pinned to npm:@base44/sdk@0.8.34. Frontend package.json pins @base44/sdk 0.8.34 exactly, matching the deploy gate/backend imports. The editable surface does not expose package-lock; the observed package layer remains ^0.8.42 / 0.8.42, so regeneration/alignment is external proof.
...
npm run check:base44-functions must run before Base44 Save & Deploy to catch function syntax, duplicate declarations, deploy-risk _shared imports, committed email literals, and getQuestions marker/projection diagnostics before manual backend publish. Health PASS is not release-ready proof. Health is a contract guard, not release proof. A4 Health automatically proves the exact package.json SDK spec only; package-lock resolution, live backend deployment, production VAPID provisioning, DB unique/index setup, real devices, store/release evidence, and multi-account RLS behavior remain explicit manual/external gates.
