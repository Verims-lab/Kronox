...
- sendQuestionAnalyticsReportEmail live deploy is proven by triggering the function and reading reportBuildMarker (current: Codex323), templateVersion nine-section-email-v1, emailBodyMode nine_section_email_body, reportDeliveryMode email_body_only, bodyContainsExactlyRequiredSections true, requiredSectionOrderValid true, renderedSectionHeaderCount 9, and bodyLength > 1000. A published frontend that does not change reportBuildMarker means the executed backend function did not redeploy.
...
- Game question loading first attempts online getQuestions when online or network state is unknown; guest/no-auth Solo uses the public-safe minimal projection, empty local cache is not offline, stale cache is invalidated by question-runtime-v5-per-category-projection, Retry re-fetches online, and false offline/no-cache is reserved for known offline plus failed fetch plus no usable cache. Gameplay fetches request the v2 per-category projection explicitly; getQuestions fetches numeric/string main_category_id and category_id variants per active Category before any final projection cap, returns safe v2 projectionDiagnostics, uses fallback IDs only when Category read fails, and Question category fields are not capped to the original 1-6 seed set.
- Solo question selection reads current-user active valid Category preferences before attempt start when signed in. Game.jsx explicitly calls getValidActiveSelectedCategoryIds(preferences, activeCategories) in the Solo-only path. No login/no saved preferences/empty preferences use all active categories. Category preference save validation remains separate from gameplay start. Insufficient preferences also use all active categories. Saved preferences target 70% selected categories / 30% full eligible pool only when at least 3 active valid preferences exist; this is soft weighting with fallback. The selected-category 70% lane is not difficulty-1 restricted; the global 30% lane prefers difficulty 1 from the full eligible pool where possible and safely falls back when difficulty-1 global candidates are insufficient.
