...
- B6 HealthCenter Intelligence removes obsolete comment-scan/duplicate cases, adds grouped on-demand packs, proof-quality and fix-owner reporting, duplicate-run prevention, completed pack summaries, and close/unmount cleanup. Scheduled/continuous monitoring remains external. Committed function.jsonc is the automation source of truth; Base44 automations deploy atomically with their function, dashboard changes are overwritten by the next local deploy, and there is no two-way sync.
...
Full architecture/performance cleanup keeps the root package.json @base44/sdk
spec exactly at 0.8.34, matching backend Deno imports. The GitHub package-lock
root and resolved SDK are also exact 0.8.34; source-connected Health validates
the committed package and lock files. Transient Friends, Online lobby, and debug copy/
