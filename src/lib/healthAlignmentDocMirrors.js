...
- Online offers Arkadaşını Davet Et (invite mode, 60s Pre-game Hourglass wait), Rastgele Eşleş (canonical random_online queue, 30s Pre-game Hourglass wait), and Duello as the final display name for the canonical same_question_duel key. Normal random and Duello share the backend randomMatchmaking function but use separate mode-scoped queue lanes and pairing locks. Token-proven guests and linked actors are compatible; self-match is forbidden. Join and poll can reconcile two waiting rows into one exactly-two-player Lobby, matched clients retry sanitized backend Lobby snapshots, and timeout performs one final backend poll before cancellation. Optional manual invite/social-player-list load failure is local to the invite card only and must never block the Online page or disable Rastgele Eşleş.
...
- Lobby route payloads are bootstrap-only. Waiting-room snapshots contain roster/config only; participant game snapshots add active deck/question state. useLobbySync and useWaitingRoomSync use backend getLobbySnapshot through one non-overlapping visibility-aware adaptive fallback poller with bounded backoff plus focus/reconnect refresh, reject stale revision/status regression, and never require direct client Lobby entity subscription. After a backend snapshot confirms starting/in_game, navigateToOnlineGameRoute builds canonical standard Online /game?online=1&lobbyId=... routing with a safe optional lobby code.
...
- Codex634 established reachable production custom-domain preflight; Codex635 hardens the remaining evidence boundary. Home OYNA may commit directly to /game or open /solo, but the Solo map is entry state only and PASS requires /game, solo-game-screen, question area, and an interaction target. Missing question-service request/success becomes a precise setup gap. Online /lobby or waiting UI is not matchmaking proof; no request, request without response, rejected response, and network failure remain distinct, and the explicit safe mutation gate never replaces a successful backend response.
...
all-active random difficulty-1/2 categories. Waiting-room clients may
...
- Online requires KRONOX_E2E_STORAGE_STATE and KRONOX_E2E_ALLOW_MATCHMAKING=true; production waiting/cancel may run as a runtime probe but requires observed matchmaking backend evidence. Duello A/B actor storage may be declared through KRONOX_E2E_STORAGE_STATE_A/B, but Duello remains MANUAL_EXTERNAL until deterministic pairing and correct-claim fixtures prove one backend winner and reconciled snapshots. Full Run does not execute E2E and E2E counters never alter Health blocker/fail/warning counts.
`;
