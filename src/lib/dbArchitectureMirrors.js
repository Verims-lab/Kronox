...
- Daily Calendar / Streak uses UserDailyQuestProgress daily_calendar:* rows: exactly 3 deterministic expected task keys per actor/server day, selected through an assignment lock and canonical-row selector. Duplicate rows cannot substitute for a missing key or complete the day/reward. recordDailyQuestProgress ignores client amount and accepts only same-actor/day backend-source proof; training levels 1-6 have no Joker/Hint spend receipt and cannot satisfy those tasks. claimDailyWheelReward records Çark çevir backend-side on successful idempotent DailyWheelSpin claim/recovery; getDailyQuestStatus reconciles only from the same-player/same-day spin. Opening/reopening the wheel, tapping SONRA, or viewing an already-claimed result does not create Daily progress. src/lib/dailyStatusCache.js marks Daily status stale after task-relevant events and useDailyQuests ignores stale status responses.
...
- Online lobby/result authority is backend-owned. Authenticated actors and token-proven completed guests call lobbyGateway with actor proof; findLobbyByCode, acceptGameInvite, startLobbyGame, and updateLobbyGameState serialize mutation with fail-closed EconomyOperationLock guards, enforce max four players and monotonic revision, and return canonical public snapshots. updateLobbyGameState commit_result derives winner/loser from terminal Lobby state, computes fixed +15/-6, reserves/reconciles OnlineMatchResult once, and publishes materialized score/leaderboard. Client code cannot create OnlineMatchResult or write User/GuestProfile/leaderboard score for Online.
