...
- UserHintInventory stores current user-owned Hint / İpucu balances per normalized user_email and is used by Mağaza Hint packages and Advantage package hint portions. Gameplay Hint consumption is not active.
- HintTransaction stores hint ledger/idempotency rows; Mağaza market_purchase rows are active for Hint grants.
...
- Mağaza Store displays real-money Diamond packages, Diamond-spend Joker packages, Diamond-spend Hint packages, Diamond-spend Advantage packages, and future KronoClub / Reklamları Kaldır sections. Real-money packages must not grant without approved IAP/payment verification. Store purchases do not grant Kronox Puan and do not affect Leaderboard; Online joker usage remains absent.
- DailyQuestDefinition is legacy/admin-only for historical/manual cleanup; current runtime ignores definition rows.
- createDailyQuestDefinition is a legacy/manual cleanup callable that can list, seed, create, and status-update definitions through an AdminUser-backed owner/admin guard; runtime and Admin UI do not depend on it.
- Supported active Daily Quest runtime type is solo_level_complete.
...
- DailyQuestDefinition.quest_key is the logical unique key for legacy/manual cleanup paths. Create/default seed skip or reject existing keys; existing duplicate rows require manual cleanup after backup.
- Günlük Görev does not require active DailyQuestDefinition rows. Runtime functions do not seed definition rows on app/Home open; getDailyQuestStatus uses the canonical code-owned solo_level_complete quest, is authenticated-or-completed-guest but not admin-only, preserves newly created rows if immediate refresh is stale, and loading/ensuring today’s quests does not grant Diamonds. Runtime stops duplicate/empty DB definition bloat; stale legacy rows require manual cleanup after backup/operator approval.
- Daily Quest Runtime v1 is active.
- UserDailyQuestProgress stores the active Daily Quest Runtime v1 selected player/day row. getDailyQuestStatus ensures 1 UTC-day canonical solo_level_complete quest idempotently, recordDailyQuestProgress increments only passed Solo level completion, and claimDailyQuestReward grants diamonds only with DiamondTransaction source daily_quest_reward. Runtime functions explicitly bind UserDailyQuestProgress, Home copy says Günlük Görevleri Yap, Elmasları Kazan!, completed guests use token-proven GuestProfile.diamonds and internal guest:<g_owner_key> keys, Daily Quest has no leaderboard impact, Daily Quest does not affect leaderboard, does not grant Kronox Puan, and one claim per quest per UTC day is enforced by progress/ledger idempotency keys. Daily Wheel remains separate from Daily Quest definitions. Daily Wheel and Daily Quest are separate. daily_wheel:<playerKey>:<YYYY-MM-DD>. daily_quest_reward:<playerKey>:<YYYY-MM-DD>:<quest_key>.
