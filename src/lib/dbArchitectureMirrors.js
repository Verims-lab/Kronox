...
- sendQuestionAnalyticsReportEmail requires admin auth, sends the full useful question analytics/product intelligence report inside the email body with a plain-text fallback, intentionally does not send a PDF attachment for now, and has no scheduled trigger.
- sendQuestionAnalyticsReportEmail actual sent email body includes Yönetici Özeti, Genel Kullanım Özeti, Solo Soru Algoritması İçin Sinyaller, Doğru Soru Tiplerini Öğrenme / İçerik Kalitesi, Joker Kullanımı Analizi, Oynanma Zamanı ve Kullanım Ritmi, Daha Uzun Oynama / Retention Sinyalleri, Soru / İçerik Aksiyonları, Önerilen Aksiyonlar, and Data Quality / Eksik Ölçüm. Category preference counts are aggregate distinct-user counts only; no user IDs or emails are exposed.
- Static inventory-style category pool sections are intentionally not rendered. Category/question rows may still be used in aggregate product signals, but the generated email must not restore old static inventory sections.
...
- Live report deploy proof reads templateVersion product-intel-email-v3, emailBodyMode full_product_intelligence_email, reportDeliveryMode email_body_only, bodyContainsProductIntelligenceSections true, and bodyLength > 1000; npm run build only proves the frontend Vite build, not live Base44 function deployment or SendEmail receipt.
