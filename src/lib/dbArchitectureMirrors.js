...
- sendQuestionAnalyticsReportEmail actual sent body includes Kategori Bazında Soru Havuzu, Kategori Bazında Kayıtlı Soru Havuzu, Kategori Tercihleri, Kategori Bazında Gösterim, Kategori İçi Soru Analizi, and Kategori Denge Sinyalleri. Category preference counts are aggregate distinct-user counts only; no user IDs or emails are exposed.
...
- Kategori Bazında Kayıtlı Soru Havuzu is sourced from active Question.list registered rows and shows category, difficulty level, question count, oldest year, and newest year. It is independent from analytics events. Kategori Bazında Gösterim remains report-period exposure analytics.
