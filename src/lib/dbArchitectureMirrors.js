...
- Profile/Solo joker balance reads use the shared getUserJokerBalances helper. Linked complete rows use a fast current-balance read; completed guests use a token-proven backend projection because internal guest keys are private. JokerTransaction is not scanned during render, and Mağaza purchase/Solo spend update or invalidate the actor-scoped cache.
- ensureUserJokerInventory grants exactly 3 mistake_shield, 3 card_swap, and 3 time_freeze once per verified linked or completed guest actor with actor-scoped idempotency keys.
...
- Solo streak4/streak5 authenticated rewards use DiamondTransaction.source solo_streak, EconomyOperationLock scope solo_streak_reward, persisted clean-answer receipts bound to actor + attempt + level, answer event-id deduplication, bounded receipt/lock retries, fixed +3/+5 amounts, and per-attempt/milestone idempotency. Levels 1-6 are visual-only; no Puan, Leaderboard, Daily, Wheel, Store, or Online mutation occurs. Guest rewards fail closed with non-reward HUD copy until reward-verifiable guest answer receipts exist.
...
- The legacy createDailyQuestDefinition callable is not deployed; runtime and Admin UI do not write definition rows.
...
- Daily Calendar / Streak uses exactly 3 deterministic UserDailyQuestProgress daily_calendar:* keys per actor/server day. getDailyQuestStatus performs one lock-guarded missing-row repair, same-player/day wheel receipt reconciliation, a bounded 420-row history read, and summary projection writes only when values change. useDailyQuests consumes the shared Daily status cache, renders cached status before refresh, and preserves cached completed tasks when a background refresh fails; only a true cache miss may enter the local loading/error/empty fallback. Duplicate rows cannot substitute for a missing key; training levels 1-6 cannot satisfy Joker/Hint tasks.
- recordDailyQuestProgress verifies exact same-actor/day source receipts: Joker/Hint ledger idempotency keys with bounded propagation retries, current QuestionAttemptEvent.event_id, and the persisted Solo lastAttemptId/pass/real-Joker fields. Valid events stale the shared Daily cache; duplicate receipts do not double-count and no Daily progress write affects Kronox Puan or Leaderboard.
- Home GÜNLÜK shortcut opens /daily; Günlük is not added to BottomNav.
- claimDailyQuestReward grants the code-owned 7-day streak reward server-side with DiamondTransaction source daily_calendar_streak_reward, direction earn, and exactly 200 Diamonds; it is actor/cycle idempotent, updates daily_calendar_* fields, does not grant Kronox Puan, and does not affect Leaderboard. Solo Streak remains separate under source solo_streak, while market purchases use direction spend. DailyPage shows only GÜNLÜK in the header, only Tamamlandı and Bugün in the legend, no renewal countdown, title-only task rows, and only 200 Elmas for the 7-day reward UI with no Gift Box icon/name.
...
- Online question selection is not affected by Solo preferences: startLobbyGame
  persists a bounded shared online_question_deck on Lobby, selected randomly
  from all active categories with difficulty 1/2 only, and Game reads that
  persisted deck instead of the Solo getQuestions buffer.
- Online lobby/result authority is backend-owned. Linked and token-proven guest actors call lobbyGateway with proof; waiting_room snapshots expose roster/config only and game snapshots add participant match state. One adaptive poller owns non-overlapping fallback recovery. Backend mutations stay fail-closed, max-four, revision-checked, and result scoring remains fixed +15/-6.
- Public Online/social DTOs expose username, sanitized avatar/status, public_ref/participant_ref, and necessary public game state only. They never expose email, display_name, provider ID, owner_key, raw guest_id, actor_key_hash, auth IDs, internal player keys, transaction IDs, or private storage metadata.
- Notification INVITE_OPENED is nonterminal and toast dismissal is visual only. Pending rows survive transient empty/error fetches and failed mutations; only confirmed accept, decline, expiry, deletion, or source invalidation closes the actionable row. User-facing network errors are recoverable Turkish copy, not raw backend/Axios messages.
- Accepted outgoing GameInvite rows are terminal history, not an unbounded feed. Bootstrap baselines old rows, a two-minute accepted-at window excludes stale replay, explicit Testing Agent/Health metadata maps to notification_suppressed, repeated accepts collapse into one lobby/batch summary, and the shared toast viewport is capped at four. Admin Integrity Snapshot reports stale/test-marked/repeated artifacts as dry-run bounded fingerprints/counts with no automatic deletion.
- The deploy gate caps Base44 at 50 function entry files. B1 keeps the repo at that ceiling by reusing adminDuplicateKeyReport. Backend Deno imports, the deploy gate, frontend package.json, and committed package-lock root/resolved package align on exact @base44/sdk 0.8.34.
...
- GFable 5 P0 unique priorities: DiamondTransaction.idempotency_key; DailyWheelSpin.idempotency_key and actor + spin_date; UserJokerInventory actor + joker_type; UserHintInventory actor; UserDailyQuestProgress idempotency_key and actor + quest_date + quest_key; JokerTransaction.idempotency_key; HintTransaction.idempotency_key; OnlineMatchResult idempotency_key and lobby + actor; SoloLeaderboardEntry.owner_key; Lobby.code; active EconomyOperationLock.lock_key.
...
- Historical duplicate cleanup evidence from 2026-07-06 is retained as history only and is superseded by the latest dry-run findings. Current affected keys remain blocked from unique-index configuration. The active task performs deep execution eligibility review only and does not invoke adminDuplicateKeyCleanup or mutate rows, balances, score, streak, reward, Daily, social, or leaderboard state.
- Permanent code-level logical unique guards (GFable 5): Base44 has no repo-level unique index support, so duplicate cleanup is not enough by itself and logical uniqueness is enforced at function/code level. query-before-create and reserve-first receipts protect DailyWheelSpin and OnlineMatchResult; EconomyOperationLock guards economy, Daily assignment/claim, and Lobby mutations and fails closed when unavailable; ledger writes re-read idempotency keys; Daily assignments select one canonical row per distinct expected key after a deterministic lock; SoloLeaderboardEntry re-reads the canonical owner row; Lobby code creation queries and locks the candidate code. Duplicate-key hits return the canonical record. Read-time dedupe remains a safety fallback, not uniqueness proof. adminDuplicateKeyReport remains the duplicate monitor, and platform unique indexes remain manual release gates after a fresh zero-duplicate dry-run.
- Paket B1 extends the existing adminDuplicateKeyReport into an AdminUser-gated, bounded, read-only Integrity Snapshot. It reports PASS/FAIL/INCOMPLETE per duplicate key, fingerprint-only samples, economy source/direction aggregates, Daily source-proof coverage, Solo streak receipts, and Online shared-deck/result counts. It performs no create/update/delete/merge/cleanup and adds no backend function at the 50-entry ceiling.
- Data Hygiene P0 uses prepare_cleanup_plan for a three-step boundary: dry-run report, deep execution eligibility review, then separate explicit admin/user-approved cleanup execution. The review is dryRun/readOnly/reviewOnly, keeps mutationOperationsEnabled and cleanupExecutionAvailable false, compares P0 runtime/ledger/progress/materialized-score signals, and returns irreversible group/candidate fingerprints, confidence, conflicts, reviewer decisions, and AUTO_SAFE_CANDIDATE/REVIEW_REQUIRED/DO_NOT_AUTOMATE counts. It never invokes cleanup. Current duplicate FAILs stay FAIL until approved execution runs and a fresh report verifies the data.
- Historical Data Hygiene Phase 1 was hard-bounded to 8 AUTO_SAFE UserDailyQuestProgress groups and 31 redundant UserDailyQuestProgress rows, with balancesMutated and scoresMutated false. The post-cleanup record leaves 4 UserDailyQuestProgress groups remain REVIEW_REQUIRED; JokerTransaction and GameInvite remain DO_NOT_AUTOMATE; UserJokerInventory and SoloLeaderboardEntry remain REVIEW_REQUIRED. Duplicate checks remain FAIL until separately approved execution and fresh verification. It is not current production proof and authorizes no further cleanup.
- Base44 deployability remains capped at 50 function entries. The repository has 28 parseable function.jsonc manifests and zero local automation declarations. Committed function.jsonc is the automation source of truth; automations deploy atomically with their function, dashboard changes are overwritten on the next local deploy, and there is no two-way sync. waitUntil is best-effort and is not accepted for critical writes; live dashboard state remains MANUAL_EXTERNAL.
- Paket B2 is frontend/runtime-only: Home demand-loads the full Daily Wheel visual and opt-in diagnostics chunks; Admin/Health stay guarded and lazy; concurrent identical Daily status reads dedupe in memory; Online/Health async owners invalidate stale work on cancel/unmount; tutorial media is popup-scoped with metadata preload. No entity, gateway authority, backend function, persistence, score, economy, Daily, Online, or Solo rule changes.
- Paket B3 reuses adminDuplicateKeyReport for an AdminUser-gated, bounded, read-only Question/Category QA mode. It reports aggregate taxonomy, difficulty, year, metadata, duplicate, and readiness risks with fingerprint-only samples; it creates no entity/function and performs no question rewrite, status change, rebalance, deletion, cleanup, gameplay, score, economy, Online, Solo, or Daily write.
- The former Admin Yayın Hazırlığı / Release Readiness checklist panel is removed. Release readiness is tracked through HealthCenter Release Gate/Security/Deployability packs, canonical release-proof docs, and manual/external evidence. No entity, function, index, schema, scheduler, cleanup, secret read, or deploy action changes; function count remains 50. Package layer, platform indexes, production VAPID secrets, RLS/multi-account, live deploy, devices, and stores remain manual/external.
- Paket B5 aligns SDK source and committed lock resolution to exact 0.8.34 while the deploy checker retains a safe missing-lock fallback. No DB/runtime authority changes; function count remains 50 and all manual gates stay open.
...
- Historical post-cleanup verification from 2026-07-06 reported zero duplicates at that time. It is not current proof: the latest production dry-run supersedes it with new P0/P1 duplicate findings, so affected platform unique-key configuration remains blocked until a separately approved cleanup and fresh zero-duplicate verification.
...
- OnlineMatchResult lobby_id + actor_key_hash unique is required where Base44 supports unique constraints.
...
- UserDailyQuestProgress.idempotency_key unique and actor + quest_date + quest_key unique are required where Base44 supports them.
- UserDailyQuestProgress actor + quest_date + status index is required where Base44 supports it.
...
- Duello V2 uses the stable internal same_question_duel key and reuses RandomMatchQueue, Lobby, startLobbyGame, updateLobbyGameState, and OnlineMatchResult at the 50-function ceiling. Queue pairing is mode-scoped and exactly two-player. Lobby stores one private bounded deck, identical opening anchors, one authoritative shared timeline, per-player correct counts and diagnostic response totals, one shared question/index/10-second deadline, private answers, bounded idempotency receipts, Sudden Death, and backend +50 / +25 / 0 terminal scoring with no speed bonus. Every round inserts the correct event into the shared timeline regardless of correctness. Public snapshots expose one synthetic-id answer-free prompt, empty used-question ids, synthetic resolved timeline rows, shared timing/state, and username-safe participant counts. Non-regressing refresh and idempotent due-state sync support reconnect recovery.
