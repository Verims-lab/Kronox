...
- Daily Calendar / Streak uses UserDailyQuestProgress daily_calendar:* rows: 3 tasks per server day, 9-day rotating task template cycle, event-based idempotent progress, and zero per-task reward_diamonds. getDailyQuestStatus reconciles Çark çevir from same-player/same-day DailyWheelSpin rows if the progress event write was missed; opening or reopening the wheel does not create Daily progress. src/lib/dailyStatusCache.js marks Daily status stale after task-relevant events and useDailyQuests ignores stale status responses.
