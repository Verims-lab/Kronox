...
- UserHintInventory stores current user-owned Hint / İpucu balances per normalized user_email or internal guest:<g_owner_key> actor key and is used by Mağaza Hint packages, Advantage package hint portions, and Solo starter/consume paths.
- HintTransaction stores hint ledger/idempotency rows; Mağaza market_purchase, Solo starter_grant, and Solo solo_use rows are active. consumeUserHint spends one Hint server-side with EconomyOperationLock, idempotency re-checks, sanitized responses, no Kronox Puan, and no leaderboard impact.
...
- Latest duplicate dry-run snapshot (2026-07-06, bounded scan windows): duplicates exist for DiamondTransaction idempotency keys (legacy starter/daily grant retries), UserJokerInventory user_email + joker_type rows (runtime dedupes on read), UserDailyQuestProgress keys (mostly legacy daily_quest:* rows handled by cleanupLegacyDailyQuests), and SoloLeaderboardEntry owner_key projection rows (getSoloLeaderboard dedupes server-side). Unique constraints for those keys are blocked until admin-approved cleanup. DailyWheelSpin (key and user/day), Lobby.code, and HintTransaction showed zero duplicates and may be configured as platform unique keys after a fresh dry-run.
