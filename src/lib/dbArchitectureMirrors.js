...
- Daily Wheel V2 can be a Diamond source and approved joker grant source; Mağaza purchase is a Diamond sink. purchaseJokerWithDiamonds is server-authoritative, ignores client price/cost, writes both ledgers with the same idempotency key, and uses best-effort rollback if ledger creation fails. Runtime duplicate-request, partial-failure, and inventory-ledger reconciliation proof remains manual.
