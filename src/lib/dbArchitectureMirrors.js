...
- sendQuestionAnalyticsReportEmail actual sent email body includes Executive Summary, Key Insights / Risk Flags, Action Items, and a PDF attachment notice. The PDF contains useful detail sections such as Kategori Bazında Soru Havuzu, Kategori Tercihleri, Kategori Bazında Gösterim, Kategori Denge Sinyalleri, top/low/wrong/easy/slow question lists, and data quality warnings. Category preference counts are aggregate distinct-user counts only; no user IDs or emails are exposed.
...
- Live report deploy proof reads templateVersion summary-pdf-v1, emailBodyMode summary_only, pdfAttachmentGenerated true, pdfAttachmentFilename ending .pdf, and pdfAttachmentContentType application/pdf; npm run build only proves the frontend Vite build, not live Base44 function deployment or SendEmail attachment receipt.
