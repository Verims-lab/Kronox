...
- Duello V1 uses the stable internal same_question_duel key and reuses RandomMatchQueue, Lobby, startLobbyGame, updateLobbyGameState, and OnlineMatchResult at the 50-function ceiling. Queue pairing is mode-scoped; Lobby game_mode same_question_duel requires exactly two players, stores one private bounded server deck, identical two-card opening anchors, per-player claimed_count, monotonic shared sequence/revision, private wrong-attempt/idempotency state, safe recent claim, and first-to-10 terminal winner. Public snapshots expose one synthetic-id active prompt without its answer year, empty used-question ids, self-only resolved timeline years under synthetic ids, and opponent counts without opponent card rows; scoring remains backend +15/-6.
