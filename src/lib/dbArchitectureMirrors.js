...
- The deploy gate caps Base44 at 50 function entry files. B1 keeps the repo at that ceiling by reusing adminDuplicateKeyReport. Backend Deno imports, the deploy gate, and frontend package.json align on exact @base44/sdk 0.8.34; the editable surface does not expose package-lock, while the observed package layer remains ^0.8.42 / 0.8.42 and requires external regeneration.
...
- Data Hygiene P0 adds prepare_cleanup_plan to the same report: dryRun/readOnly true, mutationOperationsEnabled false, cleanupExecutionAvailable false, explicitApprovalRequired true. It proposes canonical strategy, P0/P1 risk, eligibility and at most three irreversible group/candidate fingerprints per check. It never invokes cleanup. Review is step one; a separate explicit admin/user-approved execution task is step two. Current duplicate FAILs stay FAIL until that task runs and a fresh report verifies the data.
...
- Paket B5 aligns editable SDK source to exact 0.8.34 and changes the deploy checker to treat missing package-lock as external proof instead of a crash/fake pass. No DB/runtime authority changes; function count remains 50 and all manual gates stay open.
