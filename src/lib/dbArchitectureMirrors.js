...
- Solo question selection uses all active categories for guests and signed-in users with no/empty/insufficient preferences. Game.jsx explicitly resolves getValidActiveSelectedCategoryIds(preferences, activeCategories) in the Solo-only path before passing selected IDs to the deck builder. Saved preferences target 70% selected user categories and 30% full eligible pool only when at least 3 active valid preferences are available.
