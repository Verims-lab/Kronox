...
- Question loading for Game first attempts online getQuestions when online or network state is unknown. The signed-in gameplay response is an authenticated minimal playable projection; first-time guest Solo uses only the explicit capped guest_gameplay_runtime minimal projection, while admin/full-bank diagnostics still require AdminUser authorization. Empty local question cache is not offline; offline/no-cache is reserved for known offline plus failed fetch plus no usable cache. Question-set/category-projection replacements invalidate stale local cache by question-runtime-v7-getQuestions-live-marker version. Gameplay fetches request the v2 per-category projection explicitly; getQuestions fetches numeric/string main_category_id and category_id variants per active Category before any final projection cap, returns safe v2 projectionDiagnostics for authenticated gameplay plus getQuestionsRuntimeMarker for both authenticated and guest modes, uses fallback IDs only when Category read fails, and Question category fields are not capped to the original 1-6 seed set.
