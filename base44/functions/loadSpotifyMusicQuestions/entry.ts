...
Deno.serve(async (req) => {
  try {
    ...
    const songs = [
      ...
      { query: 'Aerosmith I Dont Want to Miss a Thing', year: 1998 },
      { query: 'Bon Jovi Livin on a Prayer', year: 1986 },
      { query: 'Def Leppard Pour Some Sugar on Me', year: 1987 },
      { query: 'Motley Crue Dr Feelgood', year: 1989 },
      { query: 'Poison Every Rose Has Its Thorn', year: 1990 },
      { query: 'Cinderella Dont Know What You Got', year: 1988 },
      ...
      { query: 'Bad Company Cant Get Enough', year: 1974 },
      { query: 'Foreigner Cold as Ice', year: 1977 },
      { query: 'Journey Dont Stop Believin', year: 1981 },
      ...
      { query: 'Frank Zappa Dont Eat the Yellow Snow', year: 1974 },
      ...
      { query: 'Blue Oyster Cult Dont Fear The Reaper', year: 1976 },
      ...
      { query: 'KC and the Sunshine Band Thats the Way', year: 1975 },
      ...
      { query: 'David Bowie Lets Dance', year: 1983 },
      { query: 'Blondie Call Me', year: 1980 },
      { query: 'Talking Heads Once in a Lifetime', year: 1981 },
      { query: 'The B-52s Rock Lobster', year: 1978 },
      { query: 'Radiohead Creep', year: 1992 },
      { query: 'Oasis Wonderwall', year: 1996 },
      { query: 'The White Stripes Seven Nation Army', year: 2003 },
      { query: 'Arctic Monkeys Do I Wanna Know', year: 2013 },
      { query: 'The Black Keys Tighten Up', year: 2011 },
      { query: 'Foo Fighters Everlong', year: 1997 },
      { query: 'System of a Down Chop Suey', year: 2001 },
    ];
    ...
    for (const { query, year } of songs) {
      try {
        ...
        if (track && track.preview_url) {
          questions.push({
            question: `"${track.name}" - ${track.artists?.[0]?.name || 'Unknown'}`,
            year: year,
            category: 'muzik',
            type: 'muzik',
    ...
    if (questions.length > 0) {
      ...
      return Response.json({
        ...
        message: `Successfully created ${created.length} music questions from Spotify`
      });
    }
