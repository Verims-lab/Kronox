...
const publicDuelTimelineCards = (cards: any[] = []) => (Array.isArray(cards) ? cards : [])
...
const publicDuelActiveCard = (question: any, sequence: number, canAttempt: boolean, revealYear = false) => {
...
const publicDuelRoundResult = (lobby: any, players: any[]) => {
...
const SAME_QUESTION_DUEL_TARGET = DUELLO_TARGET_CORRECT;
...
const buildInitialState = ({ players, questions, settings, activeMainCategoryIds, seed, gameMode }: { players: any[]; questions: any[]; settings: any; activeMainCategoryIds: Set<number>; seed: string; gameMode: string }) => {
  ...
  const neededCount = gameMode === SAME_QUESTION_DUEL_MODE
    ? 2 + DUELLO_MAX_QUESTIONS
    : players.length * 2 + 1;
  ...
  if (gameMode === SAME_QUESTION_DUEL_MODE) {
    ...
    return {
      ...
      onlineDeckMeta: {
        ...
        roundSeconds: DUELLO_ROUND_SECONDS,
...
Deno.serve(async (req) => {
  try {
    ...
    const settings = {
      ...
      ...(gameMode === SAME_QUESTION_DUEL_MODE ? { win_card_count: SAME_QUESTION_DUEL_TARGET, turn_duration: DUELLO_ROUND_SECONDS } : {}),
    };
    ...
    const duelTiming = buildDuelloRoundTiming(startTimestamp, { countdown: true });
    const updateData = {
      ...settings,
      game_mode: gameMode,
      status: 'starting',
      ...
      ...(gameMode === SAME_QUESTION_DUEL_MODE ? {
        duel_rules_version: DUELLO_RULES_VERSION,
        duel_match_state: DUELLO_MATCH_STATE.COUNTDOWN,
        duel_sequence: initialState.duelSequence,
        duel_round_attempts: [],
        duel_round_answers: [],
        duel_shared_timeline: initialState.sharedTimeline,
        duel_question_started_at: duelTiming.questionStartedAt,
        duel_question_deadline: duelTiming.questionDeadline,
        duel_countdown_ends_at: duelTiming.countdownEndsAt,
        duel_round_resolve_after: null,
        duel_round_result: null,
        duel_sudden_death: false,
        duel_result_type: null,
        duel_result_reason: null,
        duel_points_awarded: 0,
      ...
      last_activity_at: new Date(startTimestamp).toISOString(),
      state_revision: currentRevision + 1,
    };
