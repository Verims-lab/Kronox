...
const JOB_NAME = 'cancelStaleLobbies';
...
function getLobbyTouchedAt(row: any) {
  return parseTime(row?.last_activity_at)
    ?? parseTime(row?.updated_at)
    ?? parseTime(row?.updated_date)
    ?? parseTime(row?.created_at)
    ?? parseTime(row?.created_date)
    ?? 0;
}
...
Deno.serve(async (req: Request) => {
  ...
  const staleAfterMs = Math.max(5 * 60 * 1000, Number(body?.staleAfterMs) || DEFAULT_STALE_MS);
  const now = Date.now();
  const nowIso = new Date(now).toISOString();

  const rows = [
    ...await base44.asServiceRole.entities.Lobby.filter({ status: 'waiting' }, 'last_activity_at', limit).catch(() => []),
    ...await base44.asServiceRole.entities.Lobby.filter({ status: 'starting' }, 'last_activity_at', limit).catch(() => []),
  ];
  ...
  const staleRows = rows.filter((row: any) => {
    ...
    return now - getLobbyTouchedAt(row) >= staleAfterMs;
  });

  const updatedIds: string[] = [];
  if (!dryRun) {
    for (const row of staleRows) {
      await base44.asServiceRole.entities.Lobby.update(row.id, {
        status: 'cancelled',
        cancelled_at: nowIso,
