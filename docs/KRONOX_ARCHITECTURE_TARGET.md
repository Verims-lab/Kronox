...
- Mağaza catalog is code-side/static for real-money Diamond display packages,
  ...
  `reason: 'future_feature'`. Diamond-spend detail purchase popups are centered,
  safe-area-aware, viewport-bounded, and kept above BottomNav. Those reasons
  must drive the disabled `Yakında` state, accessibility disabled state, click
  guard, and no-grant behavior until approved IAP/payment verification exists.
  Diamond-spend
