...
* `base44/functions` contains 41 function entry files; 17 currently have
...
| Function manifests present in repo | 17 |
...
| Daily Wheel/Quest | Base44 functions and daily guard entities | High | Duplicate rewards, wrong player binding | Reward API with server-side reward source | Keep Diamond-only contract and replay claim tests | No if disabled/fallback |
