...
Data Hygiene P0 extends that same report with a three-stage boundary: dry-run duplicate report, deep execution eligibility review, then a separate explicitly approved cleanup task. `UserJokerInventory` compares the runtime maximum quantity candidate with latest ledger `balance_after` and distinct receipt deltas; any mismatch remains `REVIEW_REQUIRED`. Exact matching ledger receipts may become `AUTO_SAFE_CANDIDATE`, while conflicting Joker/Hint material fields remain `DO_NOT_AUTOMATE`. No quantity, ledger, reward, Diamond, streak, score, Daily, or leaderboard value is changed. Duplicate checks remain FAIL until approved execution is completed and freshly re-verified.
