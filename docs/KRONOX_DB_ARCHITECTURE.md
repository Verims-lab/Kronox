...
| `AdminUser` | DB-backed admin authorization source-of-truth. | Shared backend admin guard, Settings admin status check, admin-only functions, Health/test-suite gating. | Yes for admin authorization. Active rows with role `owner`/`admin` pass; disabled rows fail. | `base44/functions/_shared/adminAuth.ts`, current deployed `getQuestions` `admin_status` action, dedicated `getAdminStatus` mirror pending route proof, admin-only backend functions. | Must remain private/admin-only. Normal users cannot list global admins. No env email allowlist for authorization. | Unique `email`; `status`; `role + status`; `updated_at`. | Disable rows to remove access. Account deletion should disable/anonymize the row only through explicit owner/admin action. | Keep. | Prove unauthenticated 401, non-admin 403, active admin allowed, disabled admin blocked. |
