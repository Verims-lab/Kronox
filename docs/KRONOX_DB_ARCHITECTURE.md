...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `Category` | Stable question category lookup. | Online category UI, Solo active category whitelist, guest onboarding category metadata, `getQuestions`, `startLobbyGame`, seed function. | Yes for category metadata and active/passive state. | `OnlineChallengeScreen`, `getCategoryMetadata`, `seedQuestionCategories`, `getQuestions`, `startLobbyGame`. | Read is public-safe only for metadata. `getCategoryMetadata` is the guest-safe metadata callable and returns only current active id/name/description/status values. Guest onboarding must not render old hardcoded seed fallbacks; if current metadata cannot be loaded, show retry/error. Runtime `getQuestions` must derive active playable IDs from active `Category` rows, not an old hardcoded seed ID subset; active status aliases currently accepted by runtime are missing/blank, `a`, `active`, and `aktif`. | Unique `category_id`; `status`; `status + category_id`. | No routine deletion. Passive instead of delete. | Keep. | None. |
