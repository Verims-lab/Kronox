...
- `PlayerPresence` is the backend-owned Online/social presence source.
  ...
  with random opaque `social_*` target refs; non-routable guest presence is visible but
...
- The legacy `resetTestAccountProgress` callable is not deployed. The logged,
  previewed `adminResetUserProgress` path remains the supported AdminUser-gated
  reset surface.
...
- Duello V2 (stable internal key `same_question_duel`) reuses `RandomMatchQueue`, `Lobby`, `startLobbyGame`, `updateLobbyGameState`, and the existing Online result receipt path. Matching is mode-scoped and exactly two-player. Lobby stores a private bounded deck, identical opening anchors, one shared timeline, per-player correct counts and diagnostic correct-response totals, one shared sequence/deadline, private per-round answers, bounded idempotency receipts, Sudden Death state, pending terminal result, and backend-awarded `+50` / `+25` / `0` points. Round resolution always inserts the correct event into the shared timeline. Public snapshots project one synthetic-id answer-free active prompt, empty public used-question ids, synthetic resolved timeline rows, shared timing/state, and username-safe participant data. Adaptive refresh accepts non-regressing revisions and either participant may invoke idempotent due-state sync. No entity or backend function was added.
- Normal random Online (`random_online`) and Duello (`same_question_duel`) share the backend-owned `randomMatchmaking` function but use separate mode lanes and separate mode-scoped pairing locks. Waiting and active-lock reads are newest-first and bounded so historical released locks or expired queue rows cannot hide current actors. Each caller owns one waiting row; both `join` and backend `poll` can reconcile two existing compatible rows into one Lobby. Candidate selection rejects the same actor but intentionally permits guest/guest, guest/linked, and linked/linked pairs after the existing actor proof. Queue reads fail closed instead of converting transport/storage failures into an empty queue. Expired rows are settled, cancellation is serialized with pairing, partial Lobby handoff is compensated, and matched queue rows remain available for a bounded two-minute snapshot handoff. `RandomMatchQueue.public_username` stores only the backend-sanitized username needed to build the Lobby; actor hashes, Kronox IDs, guest proof, emails, and raw queue rows remain private. This repair adds no backend function and does not change Online score authority.
...
- Required unique keys include:
  ...
  - `UserDailyQuestProgress actor + quest_date + quest_key`
  ...
  - `UserHintInventory.user_email`
  - `HintTransaction.idempotency_key`
...
- Idempotency risk classification: DB/entity unique plus function-level guard
  is Low; function-level guard only is Medium/P1 hardening; neither DB/entity
  unique nor function-level guard is High.
...
- Daily Wheel and Daily Calendar client status reads share one cache contract:
  ...
  Daily views to re-fetch with stale-response protection. `claimDailyWheelReward`
  records active `daily_calendar:*` `Çark çevir` completion backend-side on a
...
- Daily source receipts are exact and idempotent. Joker/Hint tasks verify the
  matching ledger idempotency key after the server spend (with bounded
  read-after-write retries), answer tasks verify the current
  `QuestionAttemptEvent.event_id`, and Solo level/jokerless tasks verify the
  exact persisted `lastAttemptId`, passed flag, date, and real-Joker flag.
  Valid progress stales the shared cache; duplicate receipt keys do not add
  progress twice. None of these writes mutate Kronox Puan or Leaderboard.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `Category` | Stable question category lookup. | Online all-active shared-deck policy, Solo active category whitelist, guest onboarding category metadata, `getQuestions`, `startLobbyGame`, and admin/content-managed category rows. | Yes for category metadata and active/passive state. | `OnlineChallengeScreen`, `getCategoryMetadata`, `getQuestions`, `startLobbyGame`. | Read is public-safe only for metadata. `getCategoryMetadata` is the guest-safe metadata callable and returns only current active id/name/description/status values. Guest onboarding must not render old hardcoded seed fallbacks; if current metadata cannot be loaded, show retry/error. Runtime `getQuestions` must derive active playable IDs from active `Category` rows, not an old hardcoded seed ID subset. `seedQuestionCategories` has been removed; category creation/backfill is manual/admin content management plus runtime proof. Active status aliases currently accepted by runtime are missing/blank, `a`, `active`, and `aktif`. | Unique `category_id`; `status`; `status + category_id`. | No routine deletion. Passive instead of delete. | Keep. | None. |
| `SubCategory` | Future normalized subcategory lookup. | No current Profile Info preference UI usage; no runtime question mapping yet. | Future lookup source for normalized subcategory metadata. | Manual/admin row management expected later. | Read can be public like `Category`; create/update/delete stay admin-only. Rows store only subcategory metadata and `Category.category_id` references. | Unique `id`; `status`; `main_category_1`; `main_category_2`. | No routine deletion. Passive instead of delete. | Keep as additive prep. | Future proof needed before mapping `Question.sub_category` text to `SubCategory.id`. |
| `UserCategoryPreference` | Per-user interest preference rows for active main `Category` selections. | App-open Category preference popup, Profile > Profil Bilgileri `Kategori seçimi`, and Solo-only soft 70/30 question weighting load current user's selected categories. | Yes for user preference persistence and Solo soft weighting; not a hard question filter. | `src/lib/userCategoryPreferences.js`, `src/lib/categoryPreferenceOnboarding.js`, `src/components/settings/CategoryPreferenceOnboardingModal.jsx`, `src/components/settings/CategoryPreferencesSection.jsx`, `src/pages/Game.jsx`, `src/lib/soloQuestionEngine.js`. | Rows are user-scoped by owner email; normal users can read/update only their own rows, admin can inspect. Users below 3 active valid preferences are eligible for the optional app-open prompt only after active Category metadata loads successfully; transient load failures fail open and `Daha Sonra` can persist a defer marker. Preferences must not leak other users or affect Online/getQuestions selection. | `user_email + category_id` unique where platform supports it; `user_email + status`; `category_id`. | Unselected preferences become `status = P`; no hard delete needed for routine settings edits. | Keep. | Runtime two-account RLS proof remains needed for preference isolation. |
| `UserSubCategoryPreference` | Legacy retained per-user SubCategory preference rows from the earlier Settings phase. | Not used by the current Profile Info preference UI. Existing rows are left untouched. | Historical data only until a later migration decision. | No active Profile Info read/write path. | Keep user-scoped if accessed in future; do not expose globally. | `user_email + sub_category_id` if retained. | Leave existing rows untouched for now. | Retain/legacy. | Future migration/deletion proof needed before removing. |
| `UserJokerInventory` | Per-player current joker balances. | Lazy starter initialization, Profile `Joker Çantası`, Solo display/spend, Mağaza purchases, and Daily Wheel approved joker grants. | Yes for current owned joker counts. | `ensureUserJokerInventory`, `spendUserJoker`, purchase/wheel functions, shared `jokerInventory` helper, Profile, Solo, Market. | Linked rows use normalized email internally. Completed guests use token-proven internal `guest:<g_owner_key>` actor keys; raw guest id/token and actor key are not returned. Writes stay backend/admin/service-role. Duplicate/malformed rows must not crash Profile/Solo. | Logical unique actor key + `joker_type`; actor lookup; `updated_at`. Platform unique proof is manual. | Keep active rows; anonymize/delete with account deletion policy. | Keep/additive. | Prove cross-player isolation, guest-token rejection, and duplicate spend/grant behavior live. |
| `JokerTransaction` | Joker ledger/idempotency audit. | `starter_grant`, Solo `solo_use`, Daily Wheel grants, and Mağaza `market_purchase` rows. | Audit/ledger; `UserJokerInventory.quantity` is balance source. | Starter/spend keys use the resolved linked or token-proven internal guest actor key; `spendUserJoker` writes `quantity_delta = -1`; purchase/wheel paths write approved positive grants. | Append-only where possible; Profile does not expose/scan ledger rows; writes stay backend/admin/service-role. Starter repair must not overwrite balances or refund spends. | Logical unique `idempotency_key`; actor + time/type; reason + time. Platform unique proof is manual. | Retain for audit; archive after economy retention policy. | Keep/additive. | Cross-player RLS and parallel duplicate starter/spend/purchase proof remain manual/runtime. |
| `UserHintInventory` | Per-user current Hint / İpucu balance. | Mağaza Hint packages, Advantage package hint portions, Solo starter Hints, Solo Hint reveal spends, and read-only Profile `Joker Çantası` İpucu display. | Yes for current owned Hint counts; Solo gameplay consumption is backend-only. | `purchaseJokerWithDiamonds` grants packages; the `ensureUserHintInventory` client helper invokes consolidated `consumeUserHint` action `ensure`; action `consume` decrements after idempotency/lock checks; Profile reads only. | User can read own authenticated balance; writes stay backend/admin/service-role. Completed guests use internal actor keys through token-proven functions and receive sanitized responses only. | Unique actor key; `updated_at`. | Keep active rows with user account; anonymize/delete with account deletion. | Additive. | Prove cross-account isolation, arbitrary grant denial, and guest token gating. |
| `HintTransaction` | Hint / İpucu ledger/idempotency audit. | Mağaza `market_purchase`, Solo `starter_grant`, and Solo `solo_use` rows. | Audit/ledger; `UserHintInventory.quantity` is balance source. | `purchaseJokerWithDiamonds` writes positive grants; consolidated `consumeUserHint` action `ensure` writes starter rows and action `consume` writes `solo_use` rows with no answer-year/question-bank payload. | Append-only where possible; normal Profile does not expose/scan ledger rows; public/UI exports strip identity and internal keys. | Unique `idempotency_key`; actor/time; reason/time. | Retain for audit; archive after policy. | Additive. | Duplicate purchase/starter/spend and partial-failure proof remains manual/runtime. |
...
resolved player in one logical path and does not scan `JokerTransaction` during
render-time balance display. Linked users can use the owned-row fast read;
completed guests call the token-proven backend projection because the internal
guest economy key is intentionally not public. The slower
`ensureUserJokerInventory` self-heal path is reserved for missing/partial rows,
explicit retry/force, guest projection reads, or starter initialization.
Mağaza purchases and Solo spends return updated balances and update/invalidate
the same actor-scoped helper cache; logout clears it.
Runtime Profile performance proof still requires a live login and timing check.

Mağaza Store treats Daily Wheel V2 as a Diamond source and approved joker grant
source, while Mağaza Diamond-spend purchase is a Diamond sink. Real-money
Diamond packages are display/unavailable with `reason: 'real_money_unavailable'`
...
`HintTransaction`, writes Diamond spend plus matching joker/hint ledgers, treats
starter inventory self-heal as best-effort during purchase, and uses
best-effort rollback if ledger creation fails. `spendUserJoker` is
Solo-context-only and uses the same deploy-safe
...
joker portions, while Daily Calendar 7-day streak reward claims use
...
performs one lock-guarded, idempotent missing-row assignment repair for exactly
3 code-owned `UserDailyQuestProgress` rows per UTC server day with `quest_key`
prefixed by `daily_calendar:*`; the 9-day rotating
task template cycle is explicit code, not `DailyQuestDefinition` data. Task
progress is real-event-based and idempotent. `claimDailyWheelReward` records
active `Çark çevir` progress backend-side only after a successful idempotent
DailyWheelSpin claim/recovery, while `getDailyQuestStatus` reconciles the row
only from a confirmed same-player/same-day DailyWheelSpin receipt if that write
was missed. Opening the wheel, tapping
`SONRA`, or viewing an already-claimed result does not create Daily progress.
Per-task `reward_diamonds` is 0; progress alone never grants Diamonds. A day is
complete only when all 3 rows are complete, and missing a UTC day breaks the
computed streak. Calendar history reads are bounded to 420 rows and player
summary fields update only when projected values change. Fully append-only
assignment events remain a later migration, not a Hamle 3 rewrite.
...
DailyPage is a display-only surface for this state: the header shows only
`GÜNLÜK`, the calendar legend shows only `Tamamlandı` and `Bugün`, today task
cards show titles without description bodies or renewal countdown text, and the
7-day reward UI shows only `200 Elmas` without a Gift Box icon/name. These
copy/display rules do not change task rows, streak computation, idempotency, or
the 200-Diamond backend claim.
...
or account data.
| `DailyWheelSpin` | Daily Reward Wheel V2 claim ledger, reward selection, Gift Box package, and streak audit. | `getDailyWheelStatus` reads current day; `claimDailyWheelReward` creates a server-backed claim, updates `User.diamonds` / completed-guest `GuestProfile.diamonds` for Diamond portions, writes `UserJokerInventory` / `JokerTransaction.reason = daily_wheel` for approved joker portions, and records active Daily Calendar `Çark çevir` progress backend-side; `adminResetDailyWheelState` clears only today's test guard/idempotency state by Kronox User ID. | Audit/idempotency for Daily Wheel; `DailyWheelSpin` stores reward_type/reward_id/reward_segment_index, Gift Box contents, and the claimed day. `User.diamonds` / `GuestProfile.diamonds` remain Diamond balance sources; `UserJokerInventory` remains joker balance source. | Backend service-role functions only for claim/admin reset; user/admin read by RLS. Claim uses `daily_wheel:<playerKey>:<YYYY-MM-DD>`, checks existing spin by key/date, reserves a spin row, re-reads canonical same-player/same-day spin, re-checks the User/GuestProfile guard, re-checks `DiamondTransaction` for Diamond portions, uses `JokerTransaction` idempotency keys for joker portions, and records `UserDailyQuestProgress` with an idempotent daily_calendar key only after successful claim/recovery. Admin reset requires active AdminUser owner/admin, accepts Kronox User ID only, archives same-day DailyWheelSpin/DiamondTransaction/JokerTransaction idempotency keys, and sets `daily_wheel_auto_popup_reset_at`; it does not reverse rewards, grant rewards, affect Daily Quest, Kronox Puan, or leaderboard. | Unique idempotency is not guaranteed in repo schema; there is no repo-proven atomic upsert. Function-level guard only is Medium/P1 hardening, and race proof needs platform unique key or live parallel probe. | Unique `idempotency_key`; unique `user_email + spin_date`; `owner_key + spin_date`; `user_email + claimed_at`; `spin_date`; `reward_id`; `reward_type`. | Retain/anonymize on account deletion; admin test reset archives today's blocking wheel keys while preserving completed reward rows; archive old rows after retention policy. | Keep/additive. | N/A. |
| `DailyQuestDefinition` | Legacy/admin-only Daily Quest template rows. Daily Calendar runtime ignores them. | No app UI or deployed create callable remains. `cleanupLegacyDailyQuests` handles guarded dry-run/delete only. | Historical/manual cleanup only; not active runtime. | `base44/functions/cleanupLegacyDailyQuests/entry.ts`. | Runtime status/progress functions must not read, create, seed, or select definition rows. Cleanup must not delete unrelated data. | Historical `quest_key`; `status`; `sort_order`; `created_at`. | Stale duplicate rows require admin dry-run counts and explicit confirmation before deletion. | Keep entity until production cleanup is approved; no runtime writer. | Prove runtime Home/status calls do not create definitions; cleanup dry-run/delete proof remains manual. |
| `UserDailyQuestProgress` | Player-owned Daily Calendar / Streak per-day task progress. | `getDailyQuestStatus` serializes assignment repair and selects one canonical row for each of three deterministic expected quest keys; `recordDailyQuestProgress` accepts only same-actor/day backend-source proof; `claimDailyQuestReward` requires all three distinct keys before the 200-Diamond streak reward. | Yes for daily task progress, calendar completion, and streak reward readiness. | `base44/functions/getDailyQuestStatus`, `recordDailyQuestProgress`, `claimDailyQuestReward`, `cleanupLegacyDailyQuests`, `src/lib/dbGateway/dailyQuestGateway.js`, `src/lib/dailyQuestEvents.js`, `DailyPage`, `Game.jsx`, `ProfileEditPage`, `useDailyWheel`, `friendsApi`. | Registered users are owned by normalized email; completed guests use internal `guest:<g_owner_key>` after `guest_id + guest_token` proof. Client amount is ignored. Wheel claim, exact persisted Solo attempt, exact answer event, Joker/Hint ledger, friend, and profile source state are verified server-side; bounded retries cover ledger/event propagation; training levels 1-6 cannot prove Joker/Hint events. Receipt keys are deduped and accepted writes invalidate the shared Daily cache. | Unique `idempotency_key`; logical unique `actor + quest_date + quest_key`; actor/day lookup. `EconomyOperationLock` serializes assignment and claim when platform uniqueness is unavailable. | Retain canonical calendar rows for history/streak audit; duplicate rows are ignored for completion and reward. Cleanup deletes only legacy non-`daily_calendar:*` rows after confirmation. | Keep/additive. | Platform unique-index proof and live parallel assignment/claim probes remain manual release gates. |
| `OnlineMatchResult` | Per-actor Online score audit/idempotency receipt. | `updateLobbyGameState` action `commit_result` verifies the terminal authoritative Lobby winner, participant actor, and fixed +15/-6 rule; it reserves/reconciles one receipt before publishing materialized score. | Audit/idempotency; `User`/`GuestProfile` materialized score and sanitized leaderboard projection remain visible sources. | Backend service role only for create/update; `src/lib/applyOnlineResult.js` is a thin function caller and cannot write result, profile score, or leaderboard rows. | Authenticated and token-proven guest actors are resolved backend-side. Duplicate commits return the canonical receipt; conflicting/forged outcomes fail. Public responses omit raw actor, email, guest, owner, transaction, and internal row identifiers. | Unique `idempotency_key`; logical unique `lobby_id + actor_key_hash`; lobby/actor/result lookup. | Retain for audit/reconciliation. Archive after long retention. | Keep backend-authoritative. | Platform unique-index and live two-device partial-write recovery proof remain manual gates. |
| `Lobby` | Authoritative online lobby/game state. | Backend-owned create/join/leave/start/turn/result operations, scoped snapshot recovery, and one bounded shared Online deck. | Yes for Online match state. | `LobbyRoom`, lobby hooks/reducer/gateway, backend lobby functions. | Direct client mutation is blocked. Linked or token-proven guest actors are verified server-side. Lock + monotonic revision fail closed. Waiting-room DTOs expose roster/config only and omit player cards/current question/used ids/deck; participant game snapshots add active match state. Start writes one canonical deck. | Logical unique `code`; mutation lock key; revision/status/activity lookups. Platform uniqueness remains manual. | Cancel stale waiting lobbies; preserve finished/audit rows. | Keep backend-owned. | Live parallel join/start/turn and scoped-payload probes remain manual. |
| `GameInvite` | Pending/actionable online invite lifecycle. | Notification center, header, in-app toast, Online pending list, accept/decline/open, push function. | Yes for invite lifecycle. | `inviteApi`, `getOnlinePlayerSelection`, `useNotificationCenter`, `acceptGameInvite`, `sendGameInvitePush`, `dataRetention`. | Direct entity read/update/delete is admin-only. Backend service-role snapshots filter incoming rows by resolved `to_email`, outgoing linked rows by resolved `from_email`, and guest-host rows by private `from_actor_key_hash`; public DTOs expose opaque refs and username-safe data only. Toast dismissal must not mutate row. | `to_email + status + expires_at`; `from_email + status`; `from_actor_key_hash + status`; `lobby_id`; `status + expires_at`. | Pending past `expires_at` -> `expired`; completed/cancelled retained then archived. | Keep. | Live sender/recipient/third-account BOLA probe remains manual. |
| `FriendRequest` | Friend request and accepted friendship relation history with username-safe sender/target snapshots. | Friends page/API, notification center, accept/reject/remove functions. | Current practical social relation source. | `friendsApi`, `getOnlinePlayerSelection`, `sendFriendRequest`, `acceptFriendRequest`, `removeFriend`, notification center. | Direct entity read/update/delete is admin-only. Backend functions derive the linked actor, filter both `to_email` and `from_email` directions privately, and return opaque request refs plus username-safe fields. Accept/reject is receiver-only; cancel is sender-only. `sendFriendRequest` uses `FriendRequestOperationLock`; DB unique proof remains manual. | `to_email + status`; `from_email + status`; `from_email + to_email + status`; `updated_date`; active `FriendRequestOperationLock.lock_key` where platform supports it. | Retain relationship history; reject/cancel stale pending. | Keep. | Live sender/receiver/third-account BOLA probe remains manual. |
| `FriendRequestOperationLock` | Short-lived backend-owned duplicate-send race guard for friend request creation. | `sendFriendRequest`. | Operational lock, not public reporting data. | `sendFriendRequest`. | Admin/service-role only; stores hashed lock/sender/target keys and never raw email/provider/owner/raw guest/internal player identifiers. | Active `lock_key` logical uniqueness where platform supports it; function re-reads active locks and chooses a canonical winner because repo schema uniqueness is unproven. | Expire/stale old active locks by TTL; release after send flow. | Keep/additive. | Live parallel-send and platform unique/index proof remain manual. |
...
| `GameRecord` | Legacy/simple game completion record. | `useGameActions.saveGameRecord`, admin reset, analytics/reporting reads. | Not source of truth for current Solo level progression and no longer rendered as a Settings/Profile best-5 list. | `useGameActions`, supported admin reset, reporting functions. | User-owned read. Raw email exists but RLS protects. | `user_email + created_date`; `category`; `duration_seconds`. | Candidate archive after analytics events exist. | Keep but refactor/legacy. | Prove any future user-facing stats can use new projections/events; export historical rows. |
...
3. P1 - Idempotency keys are logical, not schema-enforced.
   - Affected entities: `DiamondTransaction`, `DailyWheelSpin`,
     `OnlineMatchResult`.
   ...
   - Safe path: unique constraints if Base44 supports them; otherwise backend
     functions with reserve-first/reconcile flows. DiamondTransaction and
     DailyWheelSpin currently have function-level guards only, so they remain
     Medium/P1 until DB uniqueness or live parallel proof is attached.
...
| --- | --- | --- | --- | --- | --- | --- |
| `questionGateway` | Load playable questions; normalize `answer -> year`; admin bank access. | Game, Solo engine, Online start, admin tooling. | `Question`, `Category`. | Active questions by category/state/limit; admin full-bank. | Auth required; normal users get a bounded minimal server attempt buffer only. Game loading must attempt online `getQuestions` when online/unknown, show loading while pending, treat no active questions as data-empty, and use offline/no-cache only for true offline or failed fetch with no cache. | Backend-first; client wrapper only. |
...
| `inviteGateway` | Load/open/accept/decline privacy-safe invite DTOs; merge active rows; TTL checks. | Header, toast, Online pending panel, Lobby. | `GameInvite`, `Lobby`, `PushSubscription`. | Backend social snapshot and public invite refs; expiry/status. | Opening/dismissing is visual only. Only confirmed accept/decline/expiry/invalidation is terminal; failed mutations preserve rows and use safe Turkish errors. | Backend functions with sanitized client wrappers. |
| `lobbyGateway` | Create/join/leave/start/update/commit-result; scoped recovery; state revisions. | Lobby UI, waiting room, Game. | `Lobby`, `OnlineMatchResult`, score projection. | Public lobby ref/code; expected revision; operation id; `waiting_room` or `game` snapshot scope. | Backend verifies linked/guest proof, authorization, capacity, status, and revision under a fail-closed lock. Waiting scope excludes active game/deck payload. | Backend-only mutations; client receives personalized public snapshots. |
| `scoringGateway` | Commit authoritative Online result and publish unified score projection. | Game result flow. | `User`, `GuestProfile`, `OnlineMatchResult`, `SoloLeaderboardEntry`. | Public lobby ref plus backend-derived actor and terminal winner. | Idempotent fixed +15/-6; forged client outcome ignored/rejected; no direct client result/score/projection writes. | Backend only. |
| `economyGateway` | Grant Diamonds, Daily Wheel V2 status/claim, grant approved wheel jokers, spend Diamonds for Mağaza joker purchases, reconcile ledger/balance. | App bootstrap, Home Daily Wheel, Mağaza, admin reset, future rewards. | `User`, `DiamondTransaction`, `DailyWheelSpin`, `UserJokerInventory`, `JokerTransaction`. | `idempotency_key`, `user_email + source`, `user_email + spin_date`, `user_email + joker_type`. | Unique keys or backend reserve-first. Purchases validate price and sufficient Diamonds server-side; wheel rewards are server-selected. | Backend preferred for grants/spends. |
...
Purpose: App-open popup and Profile Info persistence for user-selected main
Category interests.

Current status: onboarding + Profile Info only. The preference rows do not affect
...
* The Profile Info UI shows only active `Category.status = A/a` rows.
...
* `getQuestions` category fetches must tolerate numeric/string
  ...
  bounded attempt buffer.
* `getQuestions` v2 gameplay requests return small bounded responses by
  default; safe `projectionDiagnostics` require admin/debug context. Diagnostics
  ...
  deployed callable blocker; Codex418 expects backend marker
  `getQuestions-live-guest-year-diverse-v9-Codex418` from the deployed callable.
...
idempotency.
For economy idempotency, repo code provides function-level guards, not atomic
...
`UserDailyQuestProgress` assignments are serialized by a deterministic
actor/day assignment lock, re-read after settle, and select one canonical row
for each expected quest key; duplicate rows never count as additional tasks.
`SoloLeaderboardEntry` publishes filter by
...
| `OnlineMatchResult` | unique `idempotency_key` | P0 | Prevent double score application. Backend reserve/reconcile remains required until platform proof exists. |
| `OnlineMatchResult` | unique `lobby_id + actor_key_hash` | P0 | One result per verified linked/guest actor per lobby. |
| `OnlineMatchResult` | `lobby_id`, `actor_key_hash + applied_at` | P1 | Internal reconciliation/history; never public. |
...
| `DailyQuestDefinition` | unique `quest_key`; `status + sort_order`; `quest_key + created_at` | P2 | Legacy/manual cleanup template rows only. Runtime v1 does not select active definitions; if legacy rows remain, service-level duplicate guards plus manual cleanup proof are required. |
| `UserDailyQuestProgress` | unique `idempotency_key`; unique `actor + quest_date + quest_key` | P0 | One canonical actor/day/task assignment and duplicate-claim guard. Distinct expected keys, not row count, determine completion. |
| `UserDailyQuestProgress` | `actor + quest_date`, `actor + quest_date + status` | P1 | Daily status and claim/progress queries. |
...
`link_idempotency_key`), pre-link progress snapshots, trusted/admin or future
server-authoritative economy snapshots, `created_at`, and `last_seen_at`.
...
`createGuestProfile` remains public by design so first-open guest onboarding can
work without login. To reduce abuse and row bloat, the function size-limits
request bodies, allowlists accepted fields, rejects trusted client-supplied
fields, generates `guest_id`/raw token/username server-side, and records
`GuestCreationThrottle` hour/day buckets by hashed source metadata plus optional
client install id. These rows are monitoring/throttle state only; they are not
identity proof and must be trimmed after operational retention. Abandoned
...
users through `updateProfileSettings`. The UI edits only `username`, optional
private `age`, and optional private `gender`; the function stores
`username`, `username_normalized`, mirrored legacy `display_name`, and
`profile_settings_updated_at`. Authenticated writes verify `base44.auth.me()`;
guest writes verify `guest_id + raw guest token`. Existing leaderboard
projection rows update only public display fields (`display_name`, `initial`),
never `age`, `gender`, email, provider ids, or raw guest credentials.
...
- Guest `SoloLeaderboardEntry` owner key uses an internal `g_` key; linked
  accounts use the existing `u_` key and display username, with `display_name`
  kept as a mirrored legacy/projection field.
