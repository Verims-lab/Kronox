...
| `UserHintInventory` | Per-user current Hint / İpucu balance. | Mağaza Hint packages, Advantage package hint portions, Solo starter Hints, Solo Hint reveal spends, and read-only Profile `Joker Çantası` İpucu display. | Yes for current owned Hint counts; Solo gameplay consumption is active through backend functions only. | `purchaseJokerWithDiamonds` updates this row for Hint grants; `ensureUserHintInventory` initializes exactly 3 starter Hints once for authenticated and token-proven completed guest players; `consumeUserHint` decrements one Hint after idempotency/lock checks; Profile reads `quantity` only and does not initialize or mutate hints. | User can read own authenticated balance; create/update/delete should stay backend/admin/service-role so normal users cannot grant themselves hints. Completed guests use internal `guest:<g_owner_key>` actor keys through functions and receive sanitized responses only. | Unique `user_email`; `updated_at`. | Keep active rows with user account; anonymize/delete with account deletion in a later cleanup phase. | Additive. | Prove Account A cannot read/mutate Account B Hint inventory, normal users cannot create arbitrary grants, and guest token proof gates guest Hint spends. |
| `HintTransaction` | Hint / İpucu ledger/idempotency audit. | Mağaza `market_purchase`, Solo `starter_grant`, and Solo `solo_use` rows. | Audit/ledger; `UserHintInventory.quantity` is balance source. | `purchaseJokerWithDiamonds` writes positive market_purchase deltas for Hint grants; `ensureUserHintInventory` writes starter rows; `consumeUserHint` writes `solo_use` rows with `quantity_delta = -1`, `source = solo_hint`, and no answer-year/question-bank payload. | Append-only where possible; normal Profile does not expose ledger rows or scan them for display balance; writes stay backend/admin/service-role. Public/UI exports must not expose email, owner key, raw guest ID, raw guest token, or internal actor key. | Unique `idempotency_key`; `user_email + created_at`; `reason + created_at`. | Retain for audit; archive old rows after economy retention policy. | Additive. | Duplicate purchase/starter/spend and partial-failure proof remains manual/runtime. |
...
| `GameRecord` | Legacy/simple game completion record. | `useGameActions.saveGameRecord`, tests/admin reset, analytics/reporting reads. | Not source of truth for current Solo level progression and no longer rendered as a Settings/Profile best-5 list. | `useGameActions`, `runTestSuite`, admin reset, reporting functions. | User-owned read. Raw email exists but RLS protects. | `user_email + created_date`; `category`; `duration_seconds`. | Candidate archive after analytics events exist. | Keep but refactor/legacy. | Prove any future user-facing stats can use new projections/events; export historical rows. |
