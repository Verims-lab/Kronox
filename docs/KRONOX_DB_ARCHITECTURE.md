...
| `UserHintInventory` | Per-user current Hint / İpucu balance. | Mağaza Hint packages and Advantage package hint portions. | Yes for current owned Hint counts; gameplay Hint consumption is not active. | `purchaseJokerWithDiamonds` updates this row for Hint grants. | User can read own balance; create/update/delete should stay backend/admin/service-role so normal users cannot grant themselves hints. | Unique `user_email`; `updated_at`. | Keep active rows with user account; anonymize/delete with account deletion in a later cleanup phase. | Additive. | Prove Account A cannot read/mutate Account B Hint inventory and normal users cannot create arbitrary grants. |
| `HintTransaction` | Hint / İpucu ledger/idempotency audit. | Mağaza `market_purchase` rows for Hint packages and Advantage package hint portions. | Audit/ledger; `UserHintInventory.quantity` is balance source. | `purchaseJokerWithDiamonds` writes positive market_purchase deltas for Hint grants. | Append-only where possible; normal Profile does not expose ledger rows; writes stay backend/admin/service-role. | Unique `idempotency_key`; `user_email + created_at`; `reason + created_at`. | Retain for audit; archive old rows after economy retention policy. | Additive. | Duplicate purchase and partial-failure proof remains manual/runtime. |
