...
| `FriendRequest` | Friend request and accepted friendship relation history with username-safe sender/target snapshots. | Friends page/API, notification center, accept/reject/remove functions. | Current practical social relation source. | `friendsApi`, `sendFriendRequest`, `acceptFriendRequest`, `removeFriend`, notification center. | Sender/recipient/admin read/update/delete. Username add resolves target server-side and does not return target email. `sendFriendRequest` uses function-level `FriendRequestOperationLock` for duplicate-send race hardening; DB unique proof remains manual. Need two-account RLS proof. | `to_email + status`; `from_email + status`; `from_email + to_email + status`; `updated_date`; active `FriendRequestOperationLock.lock_key` where platform supports it. | Retain relationship history; reject/cancel stale pending. | Keep. | N/A. |
