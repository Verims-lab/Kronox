...
| `UserJokerInventory` | Per-user current joker balances. | Lazy starter initialization, Profile `Joker Çantası`, Solo joker balance display/spend, and Mağaza joker purchases. | Yes for current owned joker counts. | `ensureUserJokerInventory`, `spendUserJoker`, `purchaseJokerWithDiamonds`, `src/lib/jokerInventory.js`, `ProfilePage`, `Game.jsx` Solo mode, `MarketPage`. | User can read own balances; create/update/delete should stay backend/admin/service-role only so normal users cannot grant themselves jokers. Solo spend and Mağaza purchase are server-backed. Missing/partial rows self-heal for the authenticated owner using normalized `user_email`; duplicate or malformed rows must not crash Profile/Solo. Runtime two-account RLS proof remains manual. | Unique `user_email + joker_type`; `user_email`; `joker_type`; `updated_at`. | Keep active rows with user account; anonymize/delete with account deletion in a later cleanup phase. | Keep/additive. | Prove Account A cannot read/mutate Account B inventory and normal users cannot create arbitrary grants/spends/purchases. |
...
normalized user in one logical path and does not scan `JokerTransaction` during
render-time balance display. The slower `ensureUserJokerInventory` self-heal
path is reserved for missing/partial inventory rows, explicit retry/force, or
starter initialization. Mağaza purchases and Solo spends return updated balances
and update/invalidate the same user-scoped helper cache; logout clears it.
...
from DailyWheelSpin if that write was missed. Opening the wheel, tapping
...
| `Lobby` | Authoritative online lobby/game state. | Backend-owned create/join/leave/start/turn/result operations, snapshot recovery, and one bounded shared Online question deck. | Yes for online match state. | `LobbyRoom`, `useLobbySync`, `useWaitingRoomSync`, `lobbyGateway`, service functions (`findLobbyByCode`, `startLobbyGame`, `updateLobbyGameState`, `acceptGameInvite`). | Direct client mutation is blocked. Authenticated or token-proven guest actors are verified server-side. `EconomyOperationLock` plus expected monotonic revision serializes mutations and fails closed when lock infrastructure is unavailable. Start writes one canonical deck; stale revisions, nonparticipants, over-capacity joins, and non-host starts fail. Client DTOs expose opaque `public_ref`/`participant_ref`, username, sanitized avatar, status, and public game state only. | Logical unique `code`; active mutation lock key; `state_revision`; status/activity lookups. Platform uniqueness remains a manual release gate. | Mark stale waiting lobbies `cancelled`; preserve finished/audit rows. | Keep backend-owned. | Live parallel join/start/turn probes and platform unique-key proof remain manual. |
...
| `lobbyGateway` | Create/join/leave/start/update/commit-result; stale recovery; state revisions. | Lobby UI, waiting room, Game. | `Lobby`, `OnlineMatchResult`, score projection. | Public lobby ref/code; expected revision; operation id. | Backend verifies linked/guest actor proof, participant/host authorization, capacity, status, and monotonic revision under a fail-closed mutation lock. | Backend-only mutations; client receives public snapshots. |
