...
- Duello V1 (stable internal key `same_question_duel`) reuses `RandomMatchQueue`, `Lobby`, `startLobbyGame`, `updateLobbyGameState`, and the existing Online result receipt path. Queue rows and Lobby rows carry `same_question_duel`; matching is mode-scoped and exactly two-player. Lobby stores a private bounded deck, identical opening anchors, monotonic shared sequence, private per-round actor attempts, a bounded processed-operation window, per-player `claimed_count`, safe recent-claim state, and the terminal winner. Public game snapshots expose only the active shared card for this mode. No entity or backend function was added.
