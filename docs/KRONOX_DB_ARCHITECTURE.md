...
| `UserHintInventory` | Per-user current Hint / İpucu balance. | Mağaza Hint packages, Advantage package hint portions, Solo starter Hints, and Solo Hint reveal spends. | Yes for current owned Hint counts; Solo gameplay consumption is active through backend functions only. | `purchaseJokerWithDiamonds` updates this row for Hint grants; `ensureUserHintInventory` initializes exactly 3 starter Hints once for authenticated and token-proven completed guest players; `consumeUserHint` decrements one Hint after idempotency/lock checks. | User can read own authenticated balance; create/update/delete should stay backend/admin/service-role so normal users cannot grant themselves hints. Completed guests use internal `guest:<g_owner_key>` actor keys through functions and receive sanitized responses only. | Unique `user_email`; `updated_at`. | Keep active rows with user account; anonymize/delete with account deletion in a later cleanup phase. | Additive. | Prove Account A cannot read/mutate Account B Hint inventory, normal users cannot create arbitrary grants, and guest token proof gates guest Hint spends. |
