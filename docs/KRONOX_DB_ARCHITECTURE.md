...
| `DiamondTransaction` | Diamond ledger/idempotency audit. | `diamondEconomy` grants starter/daily; admin reset may reset/delete/adjust rows. | Audit/ledger; `User.diamonds` is balance source. | Client helper writes through user RLS; admin reset service-role. | Logical idempotency key not declared unique in repo. Multi-device race remains a platform limitation. | Unique `idempotency_key`; `user_email + created_at`; `user_email + source`; `source + created_at`. | Retain for audit. Archive old rows after finance/economy retention policy. | Keep but harden. | N/A. |
...
| Ledgers/audit | Durable idempotency and audit trail. | `DiamondTransaction`, `OnlineMatchResult`, `AdminMaintenanceLog`. |
...
| `economyGateway` | Grant Diamonds, spend future Diamonds, reconcile ledger/balance. | App bootstrap, admin reset, future shop/rewards. | `User`, `DiamondTransaction`. | `idempotency_key`, `user_email + source`. | Unique keys or backend reserve-first. | Backend preferred for grants/spends. |
