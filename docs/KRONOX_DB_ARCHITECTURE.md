...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `Category` | Stable question category lookup. | Online category UI, Solo active category whitelist, `getQuestions`, `startLobbyGame`, seed function. | Yes for category metadata and active/passive state. | `OnlineChallengeScreen`, `seedQuestionCategories`, `getQuestions`, `startLobbyGame`. | Read is public, write/delete admin. Public read is acceptable if only category metadata. Runtime `getQuestions` must derive active playable IDs from active `Category` rows, not an old hardcoded seed ID subset; active status aliases currently accepted by runtime are missing/blank, `a`, `active`, and `aktif`. | Unique `category_id`; `status`; `status + category_id`. | No routine deletion. Passive instead of delete. | Keep. | None. |
